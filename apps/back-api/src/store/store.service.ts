...
export class StoreService {
    ...
    async remove(id: string, businessId: string) {
        ...
        const sales = await this.prisma.sale.findFirst({
            where: { storeId: id },
        });
        if (sales) {
    ...
    async addWorkerToStore(input: AddWorkerToStoreInput, user: AuthPayload) {
        const { storeId, email } = input;

        // Verify store access
        await this.verifyStoreAccess(storeId, user);

        const worker = await this.workerService.findOneByEmain(email!);
        if (!worker) throw new Error("Worker not found");
        ...
        if (store && store.businessId !== worker.businessId) {
        ...
        return this.workerService.findOneByEmain(email!); // returns full WorkerEntity
    }
    ...
    async removeWorkerFromStore(
        ...
        const store = await this.prisma.store.findUnique({
            where: { id: storeId },
            select: {
                workers: { where: { id: workerId }, select: { id: true } },
            },
        });

        if (!store?.workers?.length) {
            throw new Error("Worker is not assigned to this store");
        }
        ...
        return {
            success: true,
            message: "Worker removed from store successfully",
        };
    }

    async getStoreWorkers(storeId: string, user: AuthPayload) {
        await this.verifyStoreAccess(storeId, user);
    ...
    async getStoreDashboardStats(
        ...
        const [
            ...
            this.prisma.worker.count({
                where: {
                    ...
                    shifts: { some: { storeId, status: "ACTIVE" } },
                },
            }),
        ...
        const completedShifts =
            shiftData.find((s) => s.status === "COMPLETED")?._count.id ?? 0;
        const activeShiftCount =
            shiftData.find((s) => s.status === "ACTIVE")?._count.id ?? 0;
        const totalShifts = shiftData.reduce((acc, s) => acc + s._count.id, 0);
        const avgSalesPerShift =
            shiftData.find((s) => s.status === "COMPLETED")?._avg?.sales ?? 0;
        ...
        return {
            ...
            shiftStats: {
                ...
                averageSalesPerShift: avgSalesPerShift ?? 0,
            },
        };
    }
    ...
    async getStoreReports(
        ...
        const [salesAgg, salesCount, topItems, workers, dailyRaw] =
            await Promise.all([
                this.prisma.sale.aggregate({
                    where: {
                        storeId,
                ...
                this.prisma.saleProduct.groupBy({
                    by: ["productId"],
                    where: { sale: { storeId, createdAt: { gte: since } } },
                    _sum: { quantity: true, price: true },
                    orderBy: { _sum: { quantity: "desc" } },
                    take: 10,
                }),
        ...
        for (const sale of dailyRaw) {
            ...
            const entry = dailyMap.get(date) ?? { sales: 0, orders: 0 };
        ...
        return {
            ...
            workerPerformance: workers.map((w) => {
                ...
                return {
                    ...
                    sales: w.sales.reduce((acc, s) => acc + s.totalAmount, 0),
                    hoursWorked,
                    completionRate:
                        w.shifts.length > 0
                            ? (completedShifts / w.shifts.length) * 100
                            : 0,
                };
            }),
            dailySales: Array.from(dailyMap.entries()).map(([date, v]) => ({
                date,
                sales: v.sales,
                orders: v.orders,
            })),
    ...
    async getStoreInventory(
        ...
        const items: any = products.map((p) => {
            ...
            else if (p.quantity < minQty) status = "LOW_STOCK";
    ...
    async generateStoreReport(
        ...
        // Verify access
        await this.verifyStoreAccess(input.storeId, user);
        ...
        if (!store || !store.business)
            throw new Error("Store or Business not found");
        ...
        if (!blobToken) throw new Error("Blob token missing");
        ...
        return { reportUrl: url, fileName, mediaId: media.id };
    }
    ...
    async getReportHistory(
        ...
        return docs.map((m) => ({
            ...
            url: m.url,
            fileName: m.pathname?.split("/").pop() || "unknown.pdf",
            storeId,
        }));
    }
    ...
    private async fetchReportData(
        ...
        switch (type) {
            ...
            case ReportType.INVENTORY: {
                const [products, adjustments, pos, tos] = await Promise.all([
            ...
            case ReportType.CLIENT_LOYALTY: {
                const [loyaltyProgram, topClients, pointsTxns] =
                    await Promise.all([
                ...
                return { loyaltyProgram, topClients, pointsTxns };
            }
  ...
  `;
      ...
      `;
            ...
            case ReportType.SALES_PERFORMANCE: {
                const totalSales = data.salesAgg?._sum?.totalAmount || 0;
                const totalDiscounts = data.discounts?._sum?.discount || 0;
                content = `
          ...
          <table>
            ...
            ${
                data.workers
                    ?.map((w: any) => {
                        const hours =
                            w.shifts?.reduce((acc: number, s: any) => {
                                if (!s.endTime) return acc;
                                return (
                                    acc +
                        ...
                        const rate =
                            w.shifts?.length > 0
                                ? ((completed / w.shifts.length) * 100).toFixed(
                                      0,
                                  )
                                : 0;
                        const avgPerShift =
                            w.shifts?.length > 0
                                ? (totalSales / w.shifts.length).toFixed(2)
                                : "0.00";
                        const initials =
                            w.fullName
                                ?.split(" ")
              ...
              `;
                    ...
                    .join("") || "<tr><td colspan='7'>No worker data</td></tr>"
            }
          </table>
        </div>
        ${
            data.workers?.some((w: any) => w.shifts?.length > 0)
                ? `
        <div class="card">
          ...
          <table>
            ...
            ${
              ...
              `;
                    ...
                    .join("") || "<tr><td colspan='6'>No shift data</td></tr>"
            }
          </table>
        </div>`
                : ""
            ...
            case ReportType.INVENTORY: {
                const inStock =
                    data.products?.filter(
                        (p: any) => p.quantity >= (p.minQuantity || 10),
                    ).length || 0;
                const lowStock =
                    data.products?.filter(
                        (p: any) =>
                ...
                const outOfStock =
                    data.products?.filter((p: any) => p.quantity === 0)
                        .length || 0;
                content = `
            ...
            ${
                data.topClients
                    ?.map((c: any) => {
                        const avgOrder =
                            c._count?.id > 0
                                ? (c._sum?.totalAmount || 0) / c._count.id
                                : 0;
                        const estPoints =
                            (c._sum?.totalAmount || 0) *
                            (data.loyaltyProgram?.pointsPerPurchase || 0);
                        return `
                <tr>
              ...
              `;
                    ...
                    .join("") || "<tr><td colspan='5'>No client data</td></tr>"
            }
          </table>
        </div>
        <div class="card">
          ...
          <table>
            ...
            ${
              ...
              `;
                    ...
                    .join("") || "<tr><td colspan='8'>No shift data</td></tr>"
            }
          </table>
        </div>
        ${
