...
export class StoreResolver {
    ...
    @Mutation(() => StoreEntity, { description: "Updates an existing store." })
    async updateStore(
    ...
    @UseGuards(JwtAuthGuard, RolesGuard)
    @Roles("business")
    @Mutation(() => StoreEntity, { description: "Deletes a store." })
    async deleteStore(
        @Args("id", { type: () => String }) id: string,
        @Context() context,
    ) {
    ...
    @UseGuards(JwtAuthGuard, RolesGuard)
    @Roles("business", "worker")
    @Mutation(() => WorkerEntity, { description: "Adds a worker to a store." })
    async addWorkerToStore(
        @Args("input") input: AddWorkerToStoreInput,
        @Context() context,
        @Args("inputWorker", { type: () => CreateWorkerInput, nullable: true })
        ...
        if (input.createNewWorker && inputWorker) {
            const existingWorker = await this.workerService.findOneByEmain(
                inputWorker.email,
            );
            if (existingWorker !== null) {
                worker = await this.storeService.addWorkerToStore(
                    { storeId: input.storeId, email: existingWorker.email },
                    user,
                );
            } else {
                // create worker first
                const newWorker = await this.workerService.create(inputWorker);
                ...
                worker = await this.workerService.findOneByEmain(
                    newWorker.email,
                );
            }
        } else {
            // attach existing worker
            worker = await this.storeService.addWorkerToStore(input, user);
        }
    ...
    async removeWorkerFromStore(
        ...
        const user = context.req.user;
        const result = await this.storeService.removeWorkerFromStore(
            storeId,
            workerId,
            user,
        );
    ...
    async getStores(@Context() context) {
        const user = context.req.user;
        if (user.role === "business") return this.storeService.findAll(user.id);
        if (user.role === "worker") {
            const worker = await this.workerService.findOne(user.id);
            if (!worker) throw new Error("Worker not found");
    ...
    @UseGuards(JwtAuthGuard, RolesGuard)
    @Roles("business", "worker")
    @Query(() => StoreStatistics, { name: "storeStatistics" })
    ...
    @UseGuards(JwtAuthGuard, RolesGuard)
    @Roles("business", "worker")
    @Query(() => StoreDashboardStatsEntity, { name: "storeDashboardStats" })
    async getStoreDashboardStats(
        @Args("storeId", { type: () => String }) storeId: string,
        @Context() context,
    ) {
        const user = context.req.user;
        return this.storeService.getStoreDashboardStats(storeId, user);
    }

    @UseGuards(JwtAuthGuard, RolesGuard)
    @Roles("business", "worker")
    @Query(() => [WorkerEntity], { name: "storeWorkers" })
    async getStoreWorkers(
        @Args("storeId", { type: () => String }) storeId: string,
        @Context() context,
    ) {
        const user = context.req.user;
        return this.storeService.getStoreWorkers(storeId, user);
    }

    @UseGuards(JwtAuthGuard, RolesGuard)
    @Roles("business", "worker")
    @Query(() => StoreReportEntity, { name: "storeReports" })
    async getStoreReports(
        @Args("storeId", { type: () => String }) storeId: string,
        @Args("period", { type: () => String, defaultValue: "week" })
        period: string,
        @Context() context,
    ) {
        const user = context.req.user;
        return this.storeService.getStoreReports(storeId, period, user);
    }

    @UseGuards(JwtAuthGuard, RolesGuard)
    @Roles("business", "worker")
    @Query(() => [ShiftEntityWorker], { name: "storeShifts" })
    async getStoreShifts(
        @Args("storeId", { type: () => String }) storeId: string,
        @Args("status", { type: () => ShiftStatus, nullable: true })
        status: ShiftStatus,
        @Context() context,
    ) {
        const user = context.req.user;
        return this.storeService.getStoreShifts(storeId, status, user);
    }

    @UseGuards(JwtAuthGuard, RolesGuard)
    @Roles("business", "worker")
    @Query(() => StoreInventoryEntity, { name: "storeInventory" })
    async getStoreInventory(
        @Args("storeId", { type: () => String }) storeId: string,
        @Context() context,
    ) {
        const user = context.req.user;
        return this.storeService.getStoreInventory(storeId, user);
    }

    @UseGuards(JwtAuthGuard, RolesGuard)
    @Roles("business", "worker")
    @Mutation(() => StoreReportResponse, {
        description: "Generates and uploads a PDF report",
    })
    async generateStoreReport(
        @Args("input") input: GenerateStoreReportInput,
        @Context() context,
    ) {
        const user = context.req.user;
        return this.storeService.generateStoreReport(input, user);
    }

    @UseGuards(JwtAuthGuard, RolesGuard)
    @Roles("business", "worker")
    @Query(() => [ReportHistoryEntity], { name: "reportHistory" })
    async getReportHistory(
        @Args("storeId") storeId: string,
        @Context() context,
    ) {
        const user = context.req.user;
        return this.storeService.getReportHistory(storeId, user);
    }

    // ── Subscriptions ──────────────────────────────────────────────────────────

    @Subscription(() => WorkerEntity, {
        filter: (payload, variables) => payload.storeId === variables.storeId,
    })
    workerAddedToStore(
        @Args("storeId", { type: () => String }) storeId: string,
    ) {
        return pubSub.asyncIterableIterator("workerAddedToStore");
    }
    ...
    workerRemovedFromStore(
        ...
        return pubSub.asyncIterableIterator("workerRemovedFromStore");
    }
}
