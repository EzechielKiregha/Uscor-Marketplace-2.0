...
export class PlatformService {
	...
	async getPlatformSettings() {
		...
		if (!settings) {
			...
			settings = await client.platformSettings.create({
				data: {
	...
	async updatePlatformSettings(input: UpdatePlatformSettingsInput) {
		...
		let updated;
		if (existing) {
			updated = await client.platformSettings.update({
				where: { id: existing.id },
				data: { ...input },
			});
		} else {
			updated = await client.platformSettings.create({
				data: { ...input },
			});
		}
		await this.pubSub.publish("PLATFORM_SETTINGS_UPDATED", {
	...
	async getPlatformMetrics() {
		...
		const totalTransactions = await client.paymentTransaction.count({
			where: { status: "COMPLETED" },
		});
		const revenueRes = await client.paymentTransaction.aggregate({
			_sum: { amount: true },
			where: { status: "COMPLETED" },
		});
		...
		const activeUsersToday = await client.client.count({
			where: {
				OR: [
		...
		const activeBusinessesToday = await client.business.count({
			where: {
				OR: [
					{ lastLogin: { gte: since24h } },
					{ createdAt: { gte: since24h } },
		...
		// KYC counts
		const [kycPendingCount, kycVerifiedCount, kycRejectedCount] =
			await Promise.all([
				client.business.count({
		...
		for (let i = 29; i >= 0; i--) {
			...
			const cnt = await client.paymentTransaction.count({
				where: {
