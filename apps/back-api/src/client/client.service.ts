...
export class ClientService {
	...
	async getClientRecommendations(clientId: string) {
		...
		const businessIds = [
			...new Set(
				orders.map((o) => o.products?.[0]?.product?.businessId).filter(Boolean),
			),
		];
	...
	async addPaymentMethod(clientId: string, input: PaymentMethodInput) {
		if (input.isDefault) {
			await this.prisma.clientPaymentMethod.updateMany({
				where: { clientId, isDefault: true },
				data: { isDefault: false },
			});
		}
		...
		if (input.type) {
			dataClause.type =
				input.type === "MOBILE_MONEY"
					...
					: input.type === "CARD"
						? PaymentMethod.CARD
						: input.type === "DEBIT_CARD"
							? PaymentMethod.CASH
							: PaymentMethod.TOKEN;
		}
		if (input.provider) {
			dataClause.provider =
				input.provider === "MTN_MOMO"
					? RechargeMethod.MTN_MONEY
					: input.type === "AIRTEL_MONEY"
						? RechargeMethod.AIRTEL_MONEY
						: input.type === "ORANGE_MONEY"
							? RechargeMethod.ORANGE_MONEY
							: input.type === "MPESA"
								? RechargeMethod.MPESA
	...
	async setDefaultPaymentMethod(paymentMethodId: string) {
		const pm = await this.prisma.clientPaymentMethod.findUnique({
		...
		return this.prisma.clientPaymentMethod.update({
			where: { id: paymentMethodId },
			data: { isDefault: true },
		});
	}
	...
	async getLoyaltyPoints(clientId: string) {
		const res = await this.prisma.pointsTransaction.aggregate({
			where: { clientId },
			_sum: { points: true },
		});
