...
export class ShiftService {
  ...
  async endShift(
    ...
    if (
      shift.workerId !== user.id &&
      user.role !== 'business'
    ) {
      throw new Error(
        'Only the shift worker or business owner can end the shift',
      )
    }
    await this.storeService.verifyStoreAccess(
      shift.storeId,
  ...
  async updateShift(
    ...
    if (user.role !== 'business') {
      throw new Error(
        'Only business owners can update shift details',
      )
    }
    await this.storeService.verifyStoreAccess(
      shift.storeId,
  ...
  async getShift(
    ...
    if (
      ...
      throw new Error(
        'Only the shift worker or business owner can view the shift',
      )
    }
    await this.storeService.verifyStoreAccess(
      shift.storeId,
  ...
  async getShiftsByWorker(
    ...
    if (
      ...
      throw new Error(
        'Only the worker or business owner can view worker shifts',
      )
    }
    const worker =
      await this.workerService.findOne(workerId)
    if (!worker) {
      throw new Error('Worker not found')
    }
    await this.storeService.verifyBusinessAccess(
      user,
    ) // Ensure user has access to business
