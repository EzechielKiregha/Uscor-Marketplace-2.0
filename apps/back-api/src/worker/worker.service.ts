...
export class WorkerService {
    ...
    const worker =
      await this.prisma.worker.create({
        data: {
          ...
          phone:
            phone ||
            Math.random().toString().slice(2, 11), // Generate random 9-digit phone if not provided
          business: {
            connect: { id: businessId },
          },
          stores: createNewWorker
            ? { connect: { id: storeId } }
            : undefined,
          kyc: kycId
            ? { connect: { id: kycId } }
  ...
  async getDashboard(
    ...
    const topSellingProducts =
      await this.prisma.saleProduct.findMany({
        where: {
          sale: {
            workerId,
            storeId,
    ...
    console.log({
      ...
      currentShift,
      salesThisWeek:
        salesThisWeek._sum.totalAmount || 0,
      salesThisMonth:
        salesThisMonth._sum.totalAmount || 0,
      topSellingProducts,
      recentOrders,
      workerPerformance: {
        totalSales:
  ...
  async getDashboardStats(
    ...
    if (currentShift) {
      ...
      currentShiftSales =
        shiftSales._sum.totalAmount || 0
      currentShiftDuration = Math.floor(
        (Date.now() -
          currentShift.startTime.getTime()) /
          1000 /
          60,
      )
    }
...
//   }
  ...
  async getWorkerReports(
    ...
    const topSellingProducts = topProducts.map(
      (item) => ({
        id: item.product?.id,
  ...
  async getWorkerPerformance(
    ...
    const [sales, shifts, salesByHour] =
      await Promise.all([
        this.prisma.sale.findMany({
          where,
          include: {
        ...
        >(
          `SELECT TO_CHAR("createdAt", 'HH24') as hour, SUM("totalAmount")::float as sales FROM "Sale" WHERE "workerId" = $1 AND "status" = 'COMPLETED' AND "createdAt" >= $2 AND "createdAt" <= $3 GROUP BY 1 ORDER BY 1`,
          workerId,
          start,
          end,
        ),
  ...
  async getWorkerSalesHistory(
    ...
    return {
      items: sales.map((sale) => ({
        ...
        client: sale.client,
        products: sale.saleProducts.map(
          (item) => ({
            id: item.product?.id,
  ...
  async createInventoryAdjustment(
    ...
    if (!productId || !storeId)
      throw new Error(
        'Store ID or Product ID is missing.',
      )
    ...
    await this.prisma.$transaction(async (tx) => {
      adjustment = tx.inventoryAdjustment.create({
        data: {
          quantity,
          adjustmentType,
          reason,
