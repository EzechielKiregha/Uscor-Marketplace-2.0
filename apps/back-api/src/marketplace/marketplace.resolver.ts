...
export class MarketplaceResolver {
	...
	async marketplaceProducts(
		...
		if (isDefaultFetch) {
			...
			const businessIdsAll = Array.from(
				new Set(itemsAll.map((p: any) => p.businessId)),
			).filter(Boolean);
			const categoryNamesAll = Array.from(
				new Set(itemsAll.map((p: any) => p.category?.name).filter(Boolean)),
			);

			const nowAll = new Date();
			const promoWhereClausesAll: any[] = [];
			if (businessIdsAll.length)
				promoWhereClausesAll.push({
					applicableBusinesses: {
						some: { id: { in: businessIdsAll } },
					},
				});
			if (categoryNamesAll.length)
				promoWhereClausesAll.push({
					applicableCategories: {
						hasSome: categoryNamesAll,
					},
				});
		...
		if (hasPromotion) {
			...
			const businessIdsAll = Array.from(
				new Set(allMatches.map((p: any) => p.businessId)),
			).filter(Boolean);
			const categoryNamesAll = Array.from(
				new Set(allMatches.map((p: any) => p.category?.name).filter(Boolean)),
			);
		...
		const businessIds = Array.from(
			new Set(items.map((p: any) => p.businessId)),
		).filter(Boolean);
		const categoryNames = Array.from(
			new Set(items.map((p: any) => p.category?.name).filter(Boolean)),
		);
	...
	@Query(() => PaginatedFreelanceServicesResponse, {
	...
	async searchMarketplace(@Args("query") query: string) {
		...
		const businessIds = Array.from(
			new Set(products.map((p: any) => p.businessId)),
		).filter(Boolean);
		const categoryNames = Array.from(
