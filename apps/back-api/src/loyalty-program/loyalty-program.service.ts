...
export class LoyaltyService {
  ...
  async createLoyaltyProgram(input: CreateLoyaltyProgramInput, user: { id: string; role: string }) {
    const { businessId, name, description, pointsPerPurchase, minimumPointsToRedeem } = input;
  ...
  async getLoyaltyPrograms(businessId: string, user: { id: string; role: string }) {
    if (user.role !== 'business') {
  ...
  async getCustomerPoints(businessId: string, clientId: string, user: { id: string; role: string }) {
    await this.businessService.verifyBusinessAccess(businessId, user);
  ...
  async getLoyaltyAnalytics(businessId: string, period: string = 'month', user: { id: string; role: string }) {
    await this.businessService.verifyBusinessAccess(businessId, user);
    ...
    const pointsRedeemed = Math.abs(transactions.filter(t => t.points < 0).reduce((sum, t) => sum + t.points, 0));

    return {
      totalMembers: totalMembers.length,
      activeMembers: transactions.length > 0 ? new Set(transactions.map(t => t.clientId)).size : 0,
      pointsEarned,
      pointsRedeemed,
  ...
  async redeemPoints(input: RedeemPointsInput, user: { id: string; role: string }) {
