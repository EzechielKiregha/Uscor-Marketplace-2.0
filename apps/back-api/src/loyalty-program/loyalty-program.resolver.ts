...
export class LoyaltyResolver {
	...
	async createLoyaltyProgram(
		...
		const program = await this.loyaltyService.createLoyaltyProgram(
			input,
			context.req.user,
		);
	...
	async updateLoyaltyProgram(
		...
		const program = await this.loyaltyService.updateLoyaltyProgram(
			id,
			input,
			context.req.user,
		);
	...
	async createPointsTransaction(
		@Args("createPointsTransactionInput")
	...
	async getPointsTransactionsByProgram(
		@Args("loyaltyProgramId", {
			type: () => String,
		})
		loyaltyProgramId: string,
		@Context() context,
	) {
		return this.loyaltyService.getPointsTransactionsByProgram(
			loyaltyProgramId,
			context.req.user,
		);
	}
	...
	async getClientPointsBalance(
		...
		return this.loyaltyService.getClientPointsBalance(
			clientId,
			context.req.user,
		);
	}
	...
	async earnPoints(@Args("input") input: EarnPointsInput, @Context() context) {
	...
	async deleteLoyaltyProgram(
		...
		const program = await this.loyaltyService.getLoyaltyProgramById(
			id,
			context.req.user,
		);
		if (!program) {
