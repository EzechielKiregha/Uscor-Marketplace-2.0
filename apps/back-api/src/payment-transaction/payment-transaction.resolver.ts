...
export class PaymentTransactionResolver {
	...
	async createPaymentTransaction(
		...
		const order = await this.prisma.order.findUnique({
			where: {
				id: createPaymentTransactionInput.orderId,
			},
	...
	async updatePaymentTransaction(
		...
		if (
			!order.products.some((item) => item.product.businessId === user.id) &&
			order.clientId !== user.id
		) {
			throw new Error(
		...
		if (!order.payment)
			return new Error("This order's payment was not initialized");

		return this.paymentTransactionService.update(id, input, order.clientId);
	}
