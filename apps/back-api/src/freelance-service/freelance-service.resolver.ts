import { Resolver, Query, Mutation, Args, Context, Subscription, Int, Float, InputType, Field } from '@nestjs/graphql';
import { FreelanceServiceService } from './freelance-service.service';
import { FreelanceServiceEntity, PaginatedFreelanceServicesResponse, WorkerServiceAssignmentEntity } from './entities/freelance-service.entity';
...
export class FreelanceServiceResolver {
  ...
  async createFreelanceService(
    @Args('input') input: CreateFreelanceServiceInput,
    @Context() context,
  ) {
    const user = context.req.user;
    const service = await this.freelanceServiceService.create(input, user.id);
  ...
  @Query(() => PaginatedFreelanceServicesResponse, { name: 'freelanceServices', description: 'Retrieves freelance services with pagination and filters.' })
  async freelanceServices(
    @Args('category', { type: () => String, nullable: true }) category?: string,
    @Args('minRate', { type: () => Float, nullable: true }) minRate?: number,
    @Args('maxRate', { type: () => Float, nullable: true }) maxRate?: number,
    @Args('isHourly', { type: () => Boolean, nullable: true }) isHourly?: boolean,
  ...
  async freelanceService(@Args('id', { type: () => String }) id: string) {
  ...
  async updateFreelanceService(
    ...
    @Args('input') input: UpdateFreelanceServiceInput,
    @Context() context,
  ) {
    const user = context.req.user;
    const service = await this.freelanceServiceService.update(id, input, user.id);
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles('business')
  @Mutation(() => WorkerServiceAssignmentEntity, { description: 'Assigns a worker to a freelance service.' })
  async assignWorkerToService(
    @Args('input') input: AssignWorkerToServiceInput,
    @Context() context,
  ) {
    const user = context.req.user;
    return this.freelanceServiceService.assignWorkerToService(input, user.id);
  }
  ...
  @Query(() => [WorkerServiceAssignmentEntity], { name: 'workerServiceAssignments', description: 'Get worker assignments for a service or worker.' })
  async workerServiceAssignments(
    ...
    return this.freelanceServiceService.getWorkerAssignments(workerId, serviceId);
  }
  ...
  freelanceServiceCreated(@Args('businessId') businessId: string) {
    return pubSub.asyncIterableIterator('freelanceServiceCreated');
  }
  ...
  freelanceServiceUpdated(@Args('businessId') businessId: string) {
    return pubSub.asyncIterableIterator('freelanceServiceUpdated');
  }
}