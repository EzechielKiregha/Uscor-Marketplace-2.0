...
export class SettingsService {
	...
	async updatePaymentConfig(
		...
		const existing = await client.paymentConfig.findUnique({
			where: { businessId },
		});
		let result;
		if (existing) {
		...
		if (business)
			await this.pubSub.publish("SETTINGS_UPDATED", {
	...
	async updateHardwareConfig(
		...
		const client = this.prisma as any;
		const existing = await client.hardwareConfig.findUnique({
			where: { businessId },
		});
		let result;
		if (existing) {
	...
	async verifyKyc(businessId: string, notes?: string) {
		...
		const doc = await client.kycDocument.findFirst({
			where: { businessId },
