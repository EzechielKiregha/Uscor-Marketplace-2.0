...
export class BusinessResolver {
	...
	@Mutation(() => BusinessEntity, {
		description: "Creates a new business with hashed password.",
	})
	async createBusiness(
		@Args("createBusinessInput")
		createBusinessInput: CreateBusinessInput,
	) {
		const created = await this.businessService.create(createBusinessInput);

		// publish businessAdded event for listing subscribers
		try {
			this.pubSub.publish("businessAdded", {
				businessAdded: created,
			});
		} catch (e) {
	...
	@Query(() => BusinessEntity, {
		name: "business",
		description: "Retrieves a single business by ID.",
	})
	async getBusiness(
		@Args("id", { type: () => String })
		id: string,
	...
	async updateBusiness(
		...
		const updated = await this.businessService.update(id, input);

		// Publish update event for subscribers
		try {
			this.pubSub.publish("businessUpdated", {
				businessUpdated: updated,
			});
		} catch (e) {
