...
export class BusinessService {
    ...
    async verifyBusinessAccess(
        _businessId: string | undefined | null,
    ...
    async getBusinesses(opts: {
        ...
        if (typeof isB2BEnabled === "boolean")
            where.isB2BEnabled = isB2BEnabled;
        if (typeof isVerified === "boolean") where.isVerified = isVerified;
        if (typeof hasLoyalty === "boolean" && hasLoyalty)
            where.loyaltyPrograms = { some: {} };
        ...
        const mapped = items.map((b: any) => ({
            ...b,
            loyaltyProgram: b.loyaltyPrograms?.length
                ? {
                      id: b.loyaltyPrograms[0].id,
                      name: b.loyaltyPrograms[0].name,
                      pointsPerDollar: b.loyaltyPrograms[0].pointsPerPurchase,
                  }
                : null,
    ...
    private async getDashboardStats(
        ...
        const _startOfCurrentMonth = startOfMonth(today);
        const _endOfCurrentMonth = endOfMonth(today);
        const _startOfPreviousMonth = startOfMonth(subDays(today, 30));
        const _endOfPreviousMonth = endOfMonth(subDays(today, 30));
