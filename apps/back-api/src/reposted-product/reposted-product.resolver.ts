...
export class RepostedProductResolver {
  ...
  @Mutation(() => RepostedProductEntity, {
    description: 'Creates a reposted product.',
  })
  async createRepostedProduct(
    @Args('createRepostedProductInput')
    createRepostedProductInput: CreateRepostedProductInput,
    @Context() context,
  ) {
    const user = context.req.user
    if (
      user.id !==
      createRepostedProductInput.businessId
    ) {
      throw new Error(
        'Businesses can only repost for themselves',
      )
    }
    return this.repostedProductService.create(
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles('business')
  @Query(() => [RepostedProductEntity], {
    name: 'repostedProducts',
    description:
  ...
  async getRepostedProduct(
    @Args('id', { type: () => String })
    id: string,
    @Context() context,
  ) {
    const user = context.req.user
    return this.repostedProductService.findOne(
      id,
      user.id,
    )
  }
  ...
  @Roles('business')
  @Mutation(() => RepostedProductEntity, {
    description: 'Deletes a reposted product.',
  })
  async deleteRepostedProduct(
    @Args('id', { type: () => String })
    id: string,
    @Context() context,
  ) {
    const user = context.req.user
    return this.repostedProductService.remove(
      id,
      user.id,
    )
  }
