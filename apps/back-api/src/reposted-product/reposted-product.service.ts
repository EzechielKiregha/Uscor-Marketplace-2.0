...
export class RepostedProductService {
	...
	async create(createRepostedProductInput: CreateRepostedProductInput) {
		const { productId, businessId, markupPercentage } =
		...
		const isOriginalEligible = await this.checkBusinessEligibility(
			product.businessId,
		);
	...
	async remove(id: string, businessId: string) {
		const _repostedProduct = await this.findOne(id, businessId);
