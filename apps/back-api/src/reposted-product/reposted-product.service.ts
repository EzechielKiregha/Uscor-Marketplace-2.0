...
export class RepostedProductService {
  ...
  async create(
    ...
    if (!isEligible) {
      throw new Error(
        'Business is not eligible for reposting',
      )
    }
    ...
    if (!isOriginalEligible) {
      throw new Error(
        'Original business is not eligible for reposting',
      )
    }
    ...
    if (existingRepost) {
      throw new Error(
        'Business has already reposted this product',
      )
    }
  ...
  async findOne(id: string, businessId: string) {
    ...
    if (
      repostedProduct.businessId !== businessId
    ) {
      throw new Error(
        'Access restricted to reposting business',
      )
    }
  ...
  async remove(id: string, businessId: string) {
    const repostedProduct = await this.findOne(
      id,
      businessId,
    )
