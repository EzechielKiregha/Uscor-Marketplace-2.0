...
export class SaleService {
    ...
    async create(createSaleInput: CreateSaleInput, user: AuthPayload) {
        ...
        if (saleProducts) {
            for (const sp of saleProducts) {
                ...
                if (!p)
                    throw new Error("This product does not exist this store");
                calculatedTotal = sumPrecise([calculatedTotal, lineTotal(p.price, sp.quantity)]);
        ...
        const finalTotal = sumPrecise([calculatedTotal, -(discount || 0)]);
        ...
        if (paymentMethod === "TOKEN") {
            ...
            if (usdToTokens(balance.totalAmount) < finalTotal) {
        ...
        const sale = await this.prisma.$transaction(async (tx) => {
            ...
            for (const sp of createdSale.saleProducts) {
                ...
                if (reOwnedProduct) {
                    const markup = sumPrecise([reOwnedProduct.newPrice, -reOwnedProduct.oldPrice]);
                    if (markup > 0) {
                        const profitShare = mulPrecise(mulPrecise(markup, 0.2), sp.quantity); // 20% of markup
                ...
                if (repostedProduct) {
                    const commission = mulPrecise(calcCommission(sp.price, REPOST_COMMISSION_RATE.toNumber()), sp.quantity); // 0.2% commission
                    await tx.tokenTransaction.create({
                        data: {
    ...
    async update(
        ...
        if (saleProducts || discount !== undefined) {
            ...
            finalTotal = sumPrecise([
                calculatedTotal,
                -(discount !== undefined ? discount : sale.discount),
            ]);
        }
    ...
    async close(closeSaleInput: CloseSaleInput, user: AuthPayload) {
        ...
        const closedSale = await this.prisma.$transaction(async (tx) => {
            ...
            if (!currentShift)
                throw new Error("Something went wrong while closing current sale.");
    ...
    private async processTokenPayment(
        ...
        const tokenAmount = usdToTokens(sale.totalAmount);
    ...
    async generateReceipt(input: GenerateReceiptInput, user: AuthPayload) {
        ...
        const qrData =
            "https://uscor-marketplace-2-0-front-ui.vercel.app/sale-status?id=" +
                sale.id || "";
        ...
        if (sale.clientId) {
            ...
            if (loyaltyProgram) {
                ...
                pointsEarned =
                    sale.totalAmount * loyaltyProgram.pointsPerPurchase;
            }
        }
  ...
  <div class="receipt-container">
    ...
    async addSaleProduct(
        ...
        const saleProduct = await this.prisma.$transaction(async (tx) => {
            ...
            await tx.sale.update({
                ...
                data: {
                    totalAmount: {
                        increment: lineTotal(product.price, input.quantity),
                    },
                },
            });
    ...
    async updateSaleProduct(
        ...
        if (input.quantity !== undefined) {
            ...
            if (
                quantityDiff > 0 &&
                saleProduct.product.quantity < quantityDiff
            ) {
                throw new Error("Insufficient product quantity");
            }
        ...
        // Use transaction for all database operations
        const updatedSaleProduct = await this.prisma.$transaction(
            async (tx) => {
    ...
    async removeSaleProduct(id: string, user: AuthPayload) {
        ...
        await this.prisma.$transaction(async (tx) => {
            ...
            // Update sale total
            const priceReduction = lineTotal(saleProduct.product.price, saleProduct.quantity);
            await tx.sale.update({
                where: { id: saleProduct.saleId },
                data: {
                    totalAmount: {
    ...
    async syncOfflineSales(
        ...
        // Sort by localTimestamp for consistent ordering (earliest first)
        ...
        for (const offlineSale of sorted) {
            try {
                // 1. Deduplication: check if localId already exists
                const existing = await this.prisma.sale.findUnique({
                    where: { localId: offlineSale.localId },
                });
                ...
                for (const sp of offlineSale.saleProducts) {
                    const product = await this.prisma.product.findUnique({
                        where: { id: sp.productId },
                        select: {
                            id: true,
                    ...
                    if (!product) {
                        inventoryIssues.push(
                            `Product ${sp.productId} not found`,
                        );
                        continue;
                    }

                    if (product.storeId !== offlineSale.storeId) {
                        inventoryIssues.push(
                            `Product "${product.title}" does not belong to this store`,
                        );
                        continue;
                    }
                    ...
                    if (product.quantity < sp.quantity) {
                        inventoryIssues.push(
                            `Product "${product.title}": requested ${sp.quantity}, available ${product.quantity}`,
                        );
                    }
                }
                ...
                if (inventoryIssues.length > 0) {
                    ...
                    results.push({
                        localId: offlineSale.localId,
                        status: "CONFLICT",
                        conflictDetails: inventoryIssues.join("; "),
                    });
                    continue;
                }

                // 4. Create sale in a transaction
                const createdSale = await this.prisma.$transaction(
                    async (tx) => {
                        const sale = await tx.sale.create({
                            data: {
                                store: {
                                    connect: { id: offlineSale.storeId },
                                },
                                worker: {
                                    connect: { id: offlineSale.workerId },
                                },
                                client: offlineSale.clientId
                                    ? {
                                          connect: {
                                              id: offlineSale.clientId,
                                          },
                                      }
                                    : undefined,
                                totalAmount: offlineSale.totalAmount,
                                discount: offlineSale.discount,
                                paymentMethod: offlineSale.paymentMethod as any,
                                status: "COMPLETED",
                                isOffline: true,
                                syncStatus: "SYNCED",
                                localId: offlineSale.localId,
                                localTimestamp: new Date(
                                    offlineSale.localTimestamp,
                                ),
                                deviceId: offlineSale.deviceId,
                                saleProducts: {
                                    create: offlineSale.saleProducts.map(
                                        (sp) => ({
                                            product: {
                                                connect: {
                                                    id: sp.productId,
                        ...
                        if (currentShift) {
                            await tx.shift.update({
                                ...
                                data: {
                                    sales: {
                                        increment: offlineSale.totalAmount,
                                    },
                                    transactionCount: { increment: 1 },
                                },
                            });
                        ...
                        if (worker) {
                            await tx.business.update({
                                ...
                                data: {
                                    totalProductsSold: {
                                        increment:
                                            offlineSale.saleProducts.reduce(
                                                (sum, sp) => sum + sp.quantity,
                                                0,
                                            ),
                                    },
        ...
        return { synced, failed, conflicts, duplicates, results };
    }
}
