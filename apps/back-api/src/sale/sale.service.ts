...
export class SaleService {
	...
	async generateReceipt(input: GenerateReceiptInput, user: AuthPayload) {
		...
		doc.text(`Date: ${sale.createdAt.toISOString().split("T")[0]}`, {
			align: "center",
		});
		...
		sale.saleProducts.forEach((sp) => {
			...
			doc.text(
				sp.modifiers ? JSON.stringify(sp.modifiers).replace(/"/g, "") : "-",
				200,
				doc.y,
				{ continued: true },
			);
		...
		doc.text(`Total: $${(sale.totalAmount - sale.discount).toFixed(2)}`, {
			align: "right",
		});
