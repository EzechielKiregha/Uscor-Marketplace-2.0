...
export class InventoryService {
	...
	async createPurchaseOrder(
		...
		const { businessId, storeId, supplierId, expectedDelivery, products } =
			input;
	...
	async updatePurchaseOrder(
		...
		if (input.status === "COMPLETED" && purchaseOrder.status !== "COMPLETED") {
			...
			await Promise.all(
				products.map((p) =>
					this.productService.updateStock(p.productId, {
	...
	async createTransferOrder(
		...
		// Validate stores
		const fromStore = await this.storeService.verifyStoreAccess(
			fromStoreId,
			user,
		);
		...
		if (input.status === "DELIVERED" && transferOrder.status !== "DELIVERED") {
			...
			await Promise.all(
				products.map(async (p) => {
					...
					const original = await this.productService.updateStock(p.productId, {
						quantity: {
					...
					if (existingProduct) {
						...
						await this.productService.updateStock(existingProduct.id, {
							quantity: {
