...
export class InventoryService {
  ...
  async createPurchaseOrder(
    ...
    for (const p of products) {
      ...
      if (product.storeId !== storeId) {
        throw new Error(
          `Product ${p.productId} does not belong to store ${storeId}`,
        )
      }
    }
  ...
  async updatePurchaseOrder(
    ...
    if (input.products) {
      for (const p of input.products) {
        ...
        if (
          ...
          throw new Error(
            `Product ${p.productId} does not belong to store ${purchaseOrder.storeId}`,
          )
        }
      }
    }
    ...
    if (
      ...
      const products =
        input.products || purchaseOrder.products
      await Promise.all(
        products.map((p) =>
    ...
    // Publish subscription event
    await this.pubSub.publish(
      `purchase_order_updated_${updatedPurchaseOrder.storeId}`,
      {
  ...
  async createTransferOrder(
    ...
    const { fromStoreId, toStoreId, products } =
      input
    ...
    const toStore =
      await this.storeService.findOne(toStoreId)
    if (
      fromStore.businessId !== toStore.businessId
    ) {
      throw new Error(
        'Transfer must occur within the same business',
      )
    }
    ...
    for (const p of products) {
      ...
      if (product.storeId !== fromStoreId) {
        throw new Error(
          `Product ${p.productId} does not belong to store ${fromStoreId}`,
        )
      }
    ...
    const transferOrder =
      await this.prisma.transferOrder.create({
        data: {
          fromStore: {
            connect: { id: fromStoreId },
          },
  ...
  async updateTransferOrder(
    ...
    const transferOrder =
      await this.prisma.transferOrder.findUnique({
        where: { id },
        include: {
          fromStore: true,
          toStore: true,
    ...
    if (input.products) {
      for (const p of input.products) {
        ...
        if (
          ...
          throw new Error(
            `Product ${p.productId} does not belong to store ${transferOrder.fromStoreId}`,
          )
        }
        if (product.quantity < p.quantity) {
          throw new Error(
    ...
    if (
      ...
      const products =
        input.products || transferOrder.products
          ...
          if (existingProduct) {
            ...
            await this.prisma.product.create({
              data: {
                ...
                store: {
                  connect: {
                    id: transferOrder.toStoreId,
                  },
                },
                business: {
                  connect: {
                    id: original.businessId,
                  },
                },
                category: {
                  connect: {
    ...
    // Publish subscription events for both stores
    await this.pubSub.publish(
      `transfer_order_updated_${updatedTransferOrder.fromStoreId}`,
      {
    ...
    await this.pubSub.publish(
      `transfer_order_updated_${updatedTransferOrder.toStoreId}`,
      {
        transferOrderUpdated:
          updatedTransferOrder,
      },
