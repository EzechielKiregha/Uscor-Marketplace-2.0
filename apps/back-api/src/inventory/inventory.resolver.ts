...
import { InventoryAdjustmentEntity, InventoryEntity, PaginatedInventoryResponse } from './entities/inventory.entity';
...
import { TransferOrderEntity, PaginatedTransferOrderResponse } from './entities/transfer-order.entity';
import { CreateTransferOrderInput } from './dto/create-transfer-order.input';
import { PurchaseOrderEntity, PaginatedPurchaseOrderResponse } from './entities/purchase-order.entity';
...
export class InventoryResolver {
  ...
  @Query(() => PaginatedInventoryResponse, { name: 'inventory', description: 'Retrieves inventory for a store.' })
  async getInventory(
    @Args('storeId', { nullable: true }) storeId?: string,
    @Args('productId', { nullable: true }) productId?: string,
    @Args('lowStockOnly', { nullable: true }) lowStockOnly?: boolean,
    ...
    return this.inventoryService.getInventory(
      { storeId, productId, lowStockOnly, page, limit },
      user
    );
  }
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles('business', 'worker')
  @Query(() => PaginatedPurchaseOrderResponse, { name: 'purchaseOrders', description: 'Retrieves purchase orders for a business.' })
  async getPurchaseOrders(
    ...
    return this.inventoryService.getPurchaseOrders(
      { businessId, storeId, status, startDate, endDate, page, limit },
      user
    );
  }
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles('business', 'worker')
  @Query(() => PaginatedTransferOrderResponse, { name: 'transferOrders', description: 'Retrieves transfer orders.' })
  async getTransferOrders(
    @Args('fromStoreId', { nullable: true }) fromStoreId?: string,
    @Args('toStoreId', { nullable: true }) toStoreId?: string,
    @Args('status', { nullable: true }) status?: string,
    @Args('startDate', { nullable: true }) startDate?: Date,
    @Args('endDate', { nullable: true }) endDate?: Date,
    @Args('page', { defaultValue: 1 }) page: number = 1,
    ...
    return this.inventoryService.getTransferOrders(
      { fromStoreId, toStoreId, status, startDate, endDate, page, limit },
      user
    );
  }
  ...
  async createTransferOrder(
    @Args('input') input: CreateTransferOrderInput,
    @Context() context,
  ) {
  ...
  async updateTransferOrder(
    @Args('id', { type: () => String }) id: string,
    @Args('input') input: UpdateTransferOrderInput,
    @Context() context,
  ) {
  ...
  @Subscription(() => PurchaseOrderEntity, {
    filter: (payload, variables) => {
      return payload.purchaseOrderCreated.storeId === variables.storeId;
    },
  })
  ...
  @Subscription(() => PurchaseOrderEntity, {
    filter: (payload, variables) => {
      return payload.purchaseOrderUpdated.storeId === variables.storeId;
    },
  })
  ...
  @Subscription(() => TransferOrderEntity, {
    filter: (payload, variables) => {
      return payload.transferOrderCreated.fromStoreId === variables.storeId || 
             payload.transferOrderCreated.toStoreId === variables.storeId;
    },
  })
  ...
  @Subscription(() => TransferOrderEntity, {
    filter: (payload, variables) => {
      return payload.transferOrderUpdated.fromStoreId === variables.storeId || 
             payload.transferOrderUpdated.toStoreId === variables.storeId;
    },
  })
