...
export class ReOwnedProductService {
	...
	async create(createReOwnedProductInput: CreateReOwnedProductInput) {
		...
		const isOriginalOwnerEligible = await this.checkBusinessEligibility(
	...
	async approve(
		...
		const bothApproved =
			(isOriginalOwner ? isApproved : reOwnedProduct.isNewOwnerApproved) &&
			(isNewOwner ? isApproved : reOwnedProduct.isOriginalApproved);
