...
export class ReOwnedProductService {
  ...
  async checkBusinessEligibility(
    businessId: string,
  ): Promise<boolean> {
    const business =
  ...
  async create(
    createReOwnedProductInput: CreateReOwnedProductInput,
  ) {
    const {
      originalProductId,
      newOwnerId,
    ...
    const isNewOwnerEligible =
      await this.checkBusinessEligibility(
        newOwnerId,
      )
    if (!isNewOwnerEligible) {
      throw new Error(
        'New owner is not eligible for re-ownership',
      )
    }
    ...
    const isOriginalOwnerEligible =
      await this.checkBusinessEligibility(
        originalProduct.businessId,
      )
    if (!isOriginalOwnerEligible) {
      throw new Error(
        'Original owner is not eligible for re-ownership',
      )
    }
    ...
    const reOwnedProduct =
      await this.prisma.reOwnedProduct.create({
        data: {
          newProduct: {
            connect: { id: newProduct.id },
          },
          originalProduct: {
            connect: { id: originalProductId },
          },
  ...
  async approve(
    approveReOwnedProductInput: ApproveReOwnedProductInput,
    approvingBusinessId: string,
  ) {
    const {
      reOwnedProductId,
      chatId,
    ...
    // Verify chat ID
    if (
      reOwnedProduct.agreedViaChatId !== chatId
    ) {
      throw new Error(
        'Invalid chat ID for approval',
      )
    }
    ...
    const isOriginalOwner =
      ...
      approvingBusinessId
    const isNewOwner =
      reOwnedProduct.newOwnerId ===
      approvingBusinessId
    if (!isOriginalOwner && !isNewOwner) {
      throw new Error(
        'Only involved businesses can approve',
      )
    }
    ...
    const bothApproved =
      (isOriginalOwner
        ? isApproved
        : reOwnedProduct.isNewOwnerApproved) &&
      (isNewOwner
        ? isApproved
        : reOwnedProduct.isOriginalApproved)
    if (bothApproved && isApproved) {
      // Decrement original product quantity
      await this.prisma.product.update({
        where: {
          id: reOwnedProduct.originalProductId,
        },
        data: {
          quantity: {
            decrement: reOwnedProduct.quantity,
          },
        },
      })
      ...
      if (
        reOwnedProduct.originalProduct
          .isPhysical &&
        reOwnedProduct.shippingId
      ) {
        await this.prisma.shipping.update({
          where: {
            id: reOwnedProduct.shippingId,
          },
  ...
  async createShipping(
    ...
    if (
      reOwnedProduct.newOwnerId !== businessId
    ) {
      throw new Error(
        'Only the new owner can create shipping details',
      )
    }
    if (
      !reOwnedProduct.originalProduct.isPhysical
    ) {
      throw new Error(
        'Shipping details only apply to physical products',
      )
    }
    if (
      !reOwnedProduct.isOriginalApproved ||
      ...
      throw new Error(
        'Both approvals required before shipping',
      )
    }

    if (!reOwnedProduct.shippingId)
      return new Error('Shipping ID is missing')
  ...
  async findOne(id: string, businessId: string) {
    ...
    if (
      reOwnedProduct.oldOwnerId !== businessId &&
      reOwnedProduct.newOwnerId !== businessId
    ) {
      throw new Error(
        'Access restricted to involved businesses',
      )
    }
    return reOwnedProduct
  }
