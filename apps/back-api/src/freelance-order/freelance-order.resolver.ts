...
export class FreelanceOrderResolver {
  ...
  async createFreelanceOrder(
    ...
    const order =
      await this.freelanceOrderService.create(
        input,
        user.id,
      )
  ...
  async updateFreelanceOrder(
    ...
    const user = context.req.user
    const order =
      await this.freelanceOrderService.update(
        id,
        input,
        user.id,
        user.role,
    ...
    if (!order)
      throw new Error(
        `Failed to update freelance order ${id}.`,
      )
  ...
  async completeFreelanceOrder(
    ...
    const user = context.req.user
    const order =
      await this.freelanceOrderService.completeOrder(
        id,
        user.id,
      )
  ...
  async releaseEscrow(
    ...
    const user = context.req.user
    const order =
      await this.freelanceOrderService.releaseEscrow(
        orderId,
        user.id,
        user.role,
      )
