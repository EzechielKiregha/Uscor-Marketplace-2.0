...
export class FreelanceOrderService {
	...
	async create(
		...
		if (paymentMethod === PaymentMethod.TOKEN) {
			const hasEnoughBalance = await this.validateBalance(
				clientId,
				escrowAmount,
			);
			if (!hasEnoughBalance) {
		...
		return this.prisma.$transaction(async (_prisma) => {
			const freelanceOrder = await this.prisma.freelanceOrder.create({
				data: {
					client: {
						connect: { id: clientId },
					},
	...
	async update(
		...
		const { status, escrowStatus, commissionPercent } =
			updateFreelanceOrderInput;
		const order = await this.findOne(id);
			...
			return this.prisma.$transaction(async (_prisma) => {
				...
				return this.prisma.freelanceOrder.update({
					...
					data: {
						...
						escrowReleasedAt:
							escrowStatus === EscrowStatus.RELEASED ? new Date() : undefined,
