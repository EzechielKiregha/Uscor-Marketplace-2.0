...
export class ChatService {
	...
	async findOne(id: string, userId?: string, _userRole?: string) {
		const chat = await this.prisma.chat.findUnique({
			where: { id },
			include: {
				product: {
		...
		const unreadCount = userId
			? chat.messages.filter((m) => !m.isRead && m.senderId !== userId).length
			: 0;
	...
	async getChatsByParticipant(participantId: string): Promise<any[]> {
		...
		return chats.map((chat) => {
			const _client = chat.participants.find((p) => p.client)?.client;
			const _business = chat.participants.find((p) => p.business)?.business;
			const _worker = chat.participants.find((p) => p.worker)?.worker;
