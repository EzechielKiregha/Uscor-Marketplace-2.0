...
export class ChatService {
  ...
  async create(createChatInput: CreateChatInput) {
    ...
    if (productId && serviceId) {
      throw new Error(
        'Chat can only be associated with a product or a service, not both',
      )
    }
    if (
      ...
      throw new Error(
        'REOWNERSHIP chats require a product',
      )
    }
    if (
      ...
      throw new Error(
        'FREELANCEORDER chats require a service',
      )
    }
    if (
      ...
      throw new Error(
        'PURCHASE chats require a product',
      )
    }
    ...
    if (productId) {
      ...
      if (
        ...
        throw new Error(
          'REOWNERSHIP chats for physical products must be secure',
        )
      }
    }
    if (serviceId) {
    ...
    if (
      ...
      throw new Error(
        'REOWNERSHIP chats require exactly two businesses',
      )
    }
    if (
      negotiationType === 'PURCHASE' &&
      ...
      throw new Error(
        'PURCHASE chats require one client and one business',
      )
    }
    ...
    if (
      ...
      throw new Error(
        'REOWNERSHIP chats must be secure',
      )
    }
  ...
  async createMessage(
    ...
    if (
      ...
      console.log(
        `Secure message logged for chat ${chatId}`,
      )
    }
  ...
  async findOne(
    ...
    const worker = chat.participants.find(
      (p) => p.worker,
    )?.worker
    ...
    const chatEntity: any = {
      ...
      negotiationType:
        chat.negotiationType as NegotiationType,
  ...
  async getChatMessages(
    ...
    const transformedMessages = items.map(
      (msg) => {
        const sender = this.getSenderInfo(
          msg.senderId,
  ...
  async getUnreadCount(
    ...
    const totalUnread = chatsWithUnread.reduce(
  ...
  private determineSenderType(
    ...
    if (participant?.clientId === senderId)
      return 'CLIENT'
    if (participant?.businessId === senderId)
      return 'BUSINESS'
    if (participant?.workerId === senderId)
      return 'WORKER'
