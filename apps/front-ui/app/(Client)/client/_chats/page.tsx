...
export default function ClientChatsPage() {
	...
	useSubscription(ON_MESSAGE_RECEIVED, {
		...
		onData: ({ client, data }) => {
			// Changed from subscriptionData to data
	...
	return (
		<div className="flex min-h-[90vh] max-h-[90vh] overflow-hidden rounded-2xl border border-orange-400/60 dark:border-orange-500/70 bg-card">
			{/* Chat List */}
			<div
				className={`${showChatList ? "flex" : "hidden"} md:flex w-full md:w-auto md:max-w-md border-r border-border flex-col`}
			>
			...
			{/* Chat Thread */}
			<div
				className={`${!showChatList ? "flex" : "hidden"} md:flex flex-1 flex flex-col min-h-0`}
			>
				{activeChatId ? (
					<ChatThread
