...
export default function LoyaltyDashboard({ client }: LoyaltyDashboardProps) {
  ...
  const {
    ...
    loading: customerPointsLoading,
    error: customerPointsError,
  } = useQuery(GET_CUSTOMER_POINTS, {
    variables: {
      businessId: activeBusiness || "",
      clientId: client.id || "",
    },
    skip: !activeBusiness || !client.id,
  });
  ...
  const businesses =
    client.orders?.reduce((acc: any[], order: any) => {
  ...
  const _handleRedeemPoints = async (points: number, benefitId: string) => {
  ...
  const totalPoints =
    customerPointsData?.customerPoints?.totalPoints ??
    ...
    return (
      <div className="bg-card border border-orange-400/60 dark:border-orange-500/70 rounded-lg p-6">
        <div className="text-center py-12">
          ...
          <Button variant="outline" onClick={() => setActiveBusiness(null)}>
  ...
  const currentTier = program?.tiers?.find(
    (tier: any) => totalPoints >= tier.minPoints,
  ) ||
    program?.tiers?.[0] || {
      id: "default",
      name: "Member",
      ...
      <div className="p-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-muted-foreground mt-1">
                  {totalPoints} / {currentTier.minPoints} points
                </p>
              </div>
            ...
            <div className="mt-4">
              ...
              <div className="w-full bg-border rounded-full h-2">
                <div
                  ...
                  style={{
                    width: `${Math.min((totalPoints / currentTier.minPoints) * 100, 100)}%`,
                  }}
                ></div>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="bg-muted rounded-lg p-4">
            ...
            <ul className="space-y-2">
              <li className="flex items-start gap-2">
                ...
                <span>
                  Earn {program.pointsPerPurchase} point
                  {program.pointsPerPurchase === 1 ? "" : "s"} for every $1
                  spent
                </span>
              </li>
              <li className="flex items-start gap-2">
          ...
          <div className="bg-muted rounded-lg p-4">
            ...
            <ul className="space-y-2">
              <li className="flex items-start gap-2">
                <ArrowRight className="h-4 w-4 text-success mt-1 shrink-0" />
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {(program.tiers || []).map((tier: any) => {
              ...
              const isNextTier =
                program.tiers.indexOf(tier) ===
                program.tiers.indexOf(currentTier) + 1;
                ...
                >
                  ...
                  <p className="text-sm text-muted-foreground mb-3">
                    {isCurrentTier
                      ...
                      : isNextTier
                        ? `Next tier benefits (only ${tier.minPoints - totalPoints} points away!)`
                  ...
                  {isNextTier && totalPoints < tier.minPoints && (
        ...
        <div className="mt-8 p-4 bg-muted rounded-lg border border-orange-400/60 dark:border-orange-500/70">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              {businesses.find((b: any) => b.id === activeBusiness)
                ?.businessType === "BOOKSTORE" && (
                <p className="text-sm text-muted-foreground mt-1">
                  As a bookstore customer, you can earn bonus points during
                  back-to-school season and redeem them for free stationery
                  items. Teachers and students get additional points on
              ...
              {businesses.find((b: any) => b.id === activeBusiness)
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Grocery shoppers can earn double points on seasonal produce
                  and redeem points for discounts on bulk purchases. Check the
                  weekly flyer for special bonus point offers.
                </p>
              )}
              ...
              {businesses.find((b: any) => b.id === activeBusiness)
                ?.businessType === "CAFE" && (
                <p className="text-sm text-muted-foreground mt-1">
                  Cafe customers get a free drink after earning 100 points.
                  Special bonus points are available during morning hours (7-10
                  AM) and for trying new menu items.
                </p>
              )}
              ...
              {businesses.find((b: any) => b.id === activeBusiness)
                ...
                businesses.find((b: any) => b.id === activeBusiness)
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    ...
                    discounts on future orders. Check back regularly for special
                    promotions that offer bonus points.
                  </p>
                )}
            </div>
          </div>
