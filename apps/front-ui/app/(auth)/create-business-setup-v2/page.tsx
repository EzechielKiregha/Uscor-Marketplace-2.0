...
const businessTypes = [
  { value: 'ARTISAN', label: 'Artisan & Handcrafted Goods', icon: Palette, color: 'from-pink-500 to-rose-500' },
  { value: 'BOOKSTORE', label: 'Bookstore & Stationery', icon: BookOpen, color: 'from-blue-500 to-indigo-500' },
  { value: 'ELECTRONICS', label: 'Electronics & Gadgets', icon: Laptop, color: 'from-cyan-500 to-blue-500' },
  { value: 'HARDWARE', label: 'Hardware & Tools', icon: Hammer, color: 'from-orange-500 to-amber-500' },
  { value: 'GROCERY', label: 'Grocery & Convenience', icon: ShoppingCart, color: 'from-green-500 to-emerald-500' },
  { value: 'CAFE', label: 'Café & Coffee Shops', icon: Coffee, color: 'from-amber-600 to-orange-600' },
  { value: 'RESTAURANT', label: 'Restaurant & Dining', icon: Utensils, color: 'from-red-500 to-rose-500' },
  { value: 'RETAIL', label: 'Retail & General Stores', icon: Store, color: 'from-purple-500 to-pink-500' },
  { value: 'BAR', label: 'Bar & Pub', icon: Beer, color: 'from-yellow-500 to-amber-500' },
  { value: 'CLOTHING', label: 'Clothing & Accessories', icon: Shirt, color: 'from-fuchsia-500 to-purple-500' },
];

// Service/Freelance specific types
const serviceTypes = [
  { value: 'CONSULTING', label: 'Consulting Services', icon: Briefcase, color: 'from-blue-500 to-cyan-500' },
  { value: 'TUTORING', label: 'Tutoring & Education', icon: BookOpen, color: 'from-indigo-500 to-purple-500' },
  { value: 'REPAIR', label: 'Repair & Maintenance', icon: Wrench, color: 'from-orange-500 to-red-500' },
  { value: 'BEAUTY', label: 'Beauty & Wellness', icon: Sparkles, color: 'from-pink-500 to-rose-500' },
  { value: 'FITNESS', label: 'Fitness & Training', icon: Dumbbell, color: 'from-green-500 to-teal-500' },
  { value: 'PHOTOGRAPHY', label: 'Photography & Video', icon: Camera, color: 'from-violet-500 to-purple-500' },
  { value: 'MUSIC', label: 'Music & Audio', icon: Music, color: 'from-red-500 to-pink-500' },
  { value: 'DESIGN', label: 'Design & Creative', icon: PenTool, color: 'from-cyan-500 to-blue-500' },
  { value: 'TECH', label: 'Tech Support & IT', icon: Laptop, color: 'from-slate-500 to-gray-500' },
  { value: 'TRANSPORT', label: 'Transportation & Delivery', icon: Truck, color: 'from-amber-500 to-orange-500' },
  { value: 'CLEANING', label: 'Cleaning Services', icon: SprayCan, color: 'from-teal-500 to-green-500' },
  { value: 'EVENTS', label: 'Event Planning', icon: Calendar, color: 'from-purple-500 to-fuchsia-500' },
];

// Sales channel options
const salesChannels = [
  { value: 'ONLINE_STORE', label: 'Online Store', desc: 'Create a customizable website', icon: Globe },
  { value: 'IN_PERSON_RETAIL', label: 'Retail Store', desc: 'Brick-and-mortar location', icon: Store },
  { value: 'IN_PERSON_EVENTS', label: 'Events & Markets', desc: 'Markets, fairs, pop-ups', icon: Calendar },
  { value: 'EXISTING_WEBSITE', label: 'Existing Website', desc: 'Add Buy Button to your site', icon: Globe },
  { value: 'SOCIAL_MEDIA', label: 'Social Media', desc: 'Facebook, Instagram, TikTok', icon: MessageSquare },
  { value: 'MARKETPLACES', label: 'Online Marketplaces', desc: 'Etsy, Amazon, Jumia', icon: ShoppingCart },
];

// Business maturity options
const businessMaturityOptions = [
  { value: 'NEW_IDEA', label: 'New Business or Idea', desc: 'Just starting out', icon: Sparkles },
  { value: 'NEW_REGISTERED', label: 'Newly Registered', desc: 'Recently established', icon: Award },
  { value: 'EXISTING', label: 'Existing Business', desc: 'Already operating', icon: Building2 },
  { value: 'EXPANDING', label: 'Expanding Business', desc: 'Growing operations', icon: TrendingUp },
];

// Platform migration options
const platformOptions = [
  { value: 'NONE', label: 'No Platform Yet', desc: 'Starting fresh', icon: Zap },
  { value: 'SHOPIFY', label: 'Shopify', desc: 'Migrate from Shopify', icon: ShoppingBag },
  { value: 'WOOCOMMERCE', label: 'WooCommerce', desc: 'Migrate from WooCommerce', icon: Globe },
  { value: 'INSTAGRAM', label: 'Instagram', desc: 'Selling on Instagram', icon: Camera },
  { value: 'FACEBOOK', label: 'Facebook', desc: 'Facebook Marketplace', icon: MessageSquare },
  { value: 'JUMIA', label: 'Jumia', desc: 'Jumia seller', icon: ShoppingCart },
  { value: 'WHATSAPP', label: 'WhatsApp Business', desc: 'Using WhatsApp for sales', icon: Smartphone },
  { value: 'OTHER', label: 'Other Platform', desc: 'Different platform', icon: Package },
];

// Product/Service type options (for product businesses)
const productTypeOptions = [
  { value: 'SELF_MADE', label: 'Self-Made Products', desc: 'Products you make or buy yourself', icon: Palette },
  { value: 'RESELL', label: 'Resale Products', desc: 'Products you buy and resell', icon: ShoppingCart },
  { value: 'DIGITAL', label: 'Digital Products', desc: 'Downloads, licenses, digital art', icon: Laptop },
  { value: 'DROPSHIPPING', label: 'Dropshipping', desc: 'Third-party fulfillment', icon: Truck },
  { value: 'PRINT_ON_DEMAND', label: 'Print-on-Demand', desc: 'Custom printed products', icon: Shirt },
  { value: 'SUBSCRIPTION', label: 'Subscription Box', desc: 'Recurring product deliveries', icon: Package },
  { value: 'DECIDE_LATER', label: 'I\'ll Decide Later', desc: 'Not sure yet', icon: Clock },
];

// Service type options (for service businesses)
const serviceOfferingOptions = [
  { value: 'ONE_TIME', label: 'One-Time Services', desc: 'Single session services', icon: CheckCircle },
  { value: 'PACKAGE', label: 'Service Packages', desc: 'Bundled service offerings', icon: Package },
  { value: 'SUBSCRIPTION', label: 'Subscription Services', desc: 'Recurring service plans', icon: Calendar },
  { value: 'CUSTOM_QUOTE', label: 'Custom Quotes', desc: 'Project-based pricing', icon: DollarSign },
  { value: 'HOURLY', label: 'Hourly Rate', desc: 'Time-based billing', icon: Clock },
  { value: 'DECIDE_LATER', label: 'I\'ll Decide Later', desc: 'Not sure yet', icon: Clock },
];

// Revenue range options
const revenueRanges = [
  { value: 'PRE_REVENUE', label: 'Pre-Revenue', desc: 'Not generating revenue yet', icon: TrendingUp },
  { value: 'UNDER_1M', label: 'Under 1M RWF', desc: 'Less than 1 million RWF/year', icon: DollarSign },
  { value: '1M_5M', label: '1M - 5M RWF', desc: '1 to 5 million RWF/year', icon: DollarSign },
  { value: '5M_20M', label: '5M - 20M RWF', desc: '5 to 20 million RWF/year', icon: DollarSign },
  { value: '20M_50M', label: '20M - 50M RWF', desc: '20 to 50 million RWF/year', icon: DollarSign },
  { value: '50M_100M', label: '50M - 100M RWF', desc: '50 to 100 million RWF/year', icon: DollarSign },
  { value: 'OVER_100M', label: 'Over 100M RWF', desc: 'More than 100 million RWF/year', icon: DollarSign },
];
...
const employeeCountOptions = [
  ...
  { value: '11_25', label: '11-25 Employees', desc: 'Medium team', icon: Users },
  { value: '26_50', label: '26-50 Employees', desc: 'Established team', icon: Users },
  { value: '50_PLUS', label: '50+ Employees', desc: 'Large organization', icon: Users },
];

// Expected growth options
const growthExpectations = [
  { value: 'STABLE', label: 'Maintain Current Size', desc: 'Happy with current scale', icon: TrendingUp },
  { value: 'MODERATE', label: 'Moderate Growth', desc: '25-50% growth expected', icon: TrendingUp },
  { value: 'AGGRESSIVE', label: 'Aggressive Growth', desc: '50-100%+ growth expected', icon: TrendingUp },
  { value: 'EXPAND_REGIONALLY', label: 'Regional Expansion', desc: 'Expand across East Africa', icon: Globe },
  { value: 'FRANCHISE', label: 'Franchise Model', desc: 'Plan to franchise', icon: Building2 },
];

// Payment method preferences
const paymentMethodOptions = [
  { value: 'MOBILE_MONEY', label: 'Mobile Money', desc: 'M-Pesa, MTN, Airtel, Orange', icon: Smartphone },
  { value: 'CASH', label: 'Cash Payments', desc: 'Physical cash transactions', icon: DollarSign },
  { value: 'CARD', label: 'Credit/Debit Cards', desc: 'Visa, Mastercard', icon: CreditCard },
  { value: 'BANK_TRANSFER', label: 'Bank Transfer', desc: 'Direct bank payments', icon: Building2 },
  { value: 'PAYPAL', label: 'PayPal', desc: 'International payments', icon: Globe },
  { value: 'CRYPTO', label: 'Cryptocurrency', desc: 'Bitcoin, USDT, etc.', icon: Zap },
];
...
const getStepsForBusinessType = (businessType: string | undefined, isService: boolean) => {
...
export default function BusinessCreatePageV2() {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      showToast("success", "Saved", "Business profile updated successfully", true, 6000, "bottom-right");
  ...
  const renderStepContent = () => {
      ...
      return (
        <div className="space-y-6">
          <div className="text-center mb-8">
            <h3 className="text-2xl font-bold text-white mb-2">What type of business are you?</h3>
          ...
          <div className="grid grid-cols-2 gap-4 mb-8">
            <button
              type="button"
              onClick={() => { setIsServiceMode(false); setSelectedBusinessType(undefined); }}
            ...
            >
              <Store className={`h-8 w-8 mx-auto mb-3 ${!isServiceMode ? 'text-primary' : 'text-gray-400'}`} />
              <h4 className={`font-semibold ${!isServiceMode ? 'text-white' : 'text-gray-300'}`}>Sell Products</h4>
              <p className="text-xs text-gray-500 mt-1">Physical or digital goods</p>
            </button>
            <button
              type="button"
              onClick={() => { setIsServiceMode(true); setSelectedBusinessType('SERVICE_FREELANCE'); }}
              className={`p-6 rounded-2xl border-2 transition-all duration-300 ${
                isServiceMode
                  ? 'bg-gradient-to-br from-primary/20 to-accent/20 border-primary/50 shadow-lg shadow-primary/10'
                  : 'bg-white/5 border-white/10 hover:bg-white/10 hover:border-primary/30'
            ...
            >
              <Briefcase className={`h-8 w-8 mx-auto mb-3 ${isServiceMode ? 'text-primary' : 'text-gray-400'}`} />
              <h4 className={`font-semibold ${isServiceMode ? 'text-white' : 'text-gray-300'}`}>Offer Services</h4>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {(isServiceMode ? serviceTypes : businessTypes).map((type) => {
              ...
              return (
                ...
                >
                  <Icon className={`h-6 w-6 mb-2 ${isSelected ? 'text-white' : 'text-gray-400 group-hover:text-primary'}`} />
                  <h4 className={`font-medium text-sm ${isSelected ? 'text-white' : 'text-gray-300'}`}>{type.label}</h4>
                </button>
              );
            })}
          </div>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="hidden">
                <FormControl>
                  <input {...field} value={selectedBusinessType || ''} onChange={() => {}} />
                </FormControl>
              </FormItem>
            )}
          />
    ...
    if (step === 2) {
      ...
      return (
        <div className="space-y-6">
          <div className="text-center mb-8">
            <h3 className="text-2xl font-bold text-white mb-2">Where will you sell?</h3>
    ...
    if (step === 3) {
      return (
        <div className="space-y-6">
          <div className="text-center mb-8">
            <h3 className="text-2xl font-bold text-white mb-2">Is this a new or existing business?</h3>
            <p className="text-gray-400">This helps us suggest the right onboarding</p>
          </div>
            ...
            {businessMaturityOptions.map((option) => {
              ...
              return (
                <button
                  key={option.value}
                  type="button"
                  onClick={() => updatePreferences('businessMaturity', option.value)}
                  className={`group p-4 rounded-xl border transition-all duration-300 text-left flex items-start gap-3 ${
                    isSelected
                      ? 'bg-gradient-to-br from-primary/20 to-accent/20 border-primary/50 shadow-lg shadow-primary/10'
                      : 'bg-white/5 border-white/10 hover:bg-white/10 hover:border-primary/30'
    ...
    if (step === 4) {
      return (
        <div className="space-y-6">
          <div className="text-center mb-8">
            <h3 className="text-2xl font-bold text-white mb-2">Do you currently sell on other platforms?</h3>
            ...
            {platformOptions.map((option) => {
              ...
              return (
                <button
                  key={option.value}
                  type="button"
                  onClick={() => updatePreferences('currentPlatform', option.value)}
                  className={`group p-4 rounded-xl border transition-all duration-300 text-left flex items-start gap-3 ${
                    isSelected
                      ? 'bg-gradient-to-br from-primary/20 to-accent/20 border-primary/50 shadow-lg shadow-primary/10'
                      : 'bg-white/5 border-white/10 hover:bg-white/10 hover:border-primary/30'
      ...
      return (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {options.map((option) => {
              const Icon = option.icon;
              const isSelected = preferences.productOrServiceType === option.value;
              return (
                <button
                  key={option.value}
                  type="button"
                  onClick={() => updatePreferences('productOrServiceType', option.value)}
                  className={`group p-4 rounded-xl border transition-all duration-300 text-left flex items-start gap-3 ${
                    isSelected
                      ? 'bg-gradient-to-br from-primary/20 to-accent/20 border-primary/50 shadow-lg shadow-primary/10'
                      : 'bg-white/5 border-white/10 hover:bg-white/10 hover:border-primary/30'
    ...
    if (step === 6) {
      return (
        <div className="space-y-6">
          <div className="text-center mb-8">
            <h3 className="text-2xl font-bold text-white mb-2">Tell us about your business size</h3>
            <p className="text-gray-400">This helps us understand your needs</p>
          </div>

          {/* Revenue Range */}
          <div className="space-y-3">
            <label className="text-sm font-medium text-gray-300">Annual Revenue (RWF)</label>
              ...
              {revenueRanges.map((option) => {
                ...
                return (
                  <button
                    key={option.value}
                    type="button"
                    onClick={() => updatePreferences('revenueRange', option.value)}
              ...
              {employeeCountOptions.map((option) => {
                ...
                return (
                  <button
                    key={option.value}
                    type="button"
                    onClick={() => updatePreferences('employeeCount', option.value)}
                  ...
                  >
                    <Users className={`h-5 w-5 mx-auto mb-1 ${isSelected ? 'text-primary' : 'text-gray-400'}`} />
    ...
    if (step === 7) {
      return (
        <div className="space-y-6">
          <div className="text-center mb-8">
            <h3 className="text-2xl font-bold text-white mb-2">What are your growth plans?</h3>
            <p className="text-gray-400">Your ambitions for the next 12 months</p>
          </div>
            ...
            {growthExpectations.map((option) => {
              ...
              return (
                <button
                  key={option.value}
                  type="button"
                  onClick={() => updatePreferences('growthExpectation', option.value)}
                  className={`group p-4 rounded-xl border transition-all duration-300 text-left flex items-start gap-3 ${
                    isSelected
                      ? 'bg-gradient-to-br from-primary/20 to-accent/20 border-primary/50 shadow-lg shadow-primary/10'
                      : 'bg-white/5 border-white/10 hover:bg-white/10 hover:border-primary/30'
    ...
    if (step === 8) {
      ...
      return (
        <div className="space-y-6">
          <div className="text-center mb-8">
            <h3 className="text-2xl font-bold text-white mb-2">Which payment methods will you accept?</h3>
            <p className="text-gray-400">Especially important for East African markets</p>
          </div>
          ...
          <div className="mt-6 p-4 rounded-xl bg-gradient-to-r from-primary/10 to-accent/10 border border-primary/20">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="font-semibold text-white text-sm">Mobile Money Integration</h4>
                <p className="text-xs text-gray-400 mt-1">
                  USCOR seamlessly integrates with M-Pesa, MTN Mobile Money, Airtel Money, and Orange Money 
                  for easy payments across East Africa.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-gray-300 font-medium">Business Name *</FormLabel>
                  <FormControl>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-gray-300 font-medium">Business Address</FormLabel>
                  <FormControl>
                    <div className="relative">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-gray-300 font-medium">Website (Optional)</FormLabel>
                  <FormControl>
                    <div className="relative">
              ...
              render={({ field }) => (
                <FormItem className="flex items-center justify-between p-4 rounded-xl bg-white/5 border border-white/10">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <FormLabel className="text-gray-300 font-medium">Enable B2B Features</FormLabel>
                      <p className="text-xs text-gray-500">Trade with other verified businesses</p>
                    </div>
                  </div>
                  <FormControl>
                    <button
      ...
      return (
        <div className="space-y-6">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-400">Review your information before finishing</p>
          </div>
          ...
          <div className="bg-gradient-to-br from-white/5 to-white/10 border border-white/10 rounded-2xl p-6 space-y-4">
            <div className="flex items-center gap-3 pb-4 border-b border-white/10">
              {selectedType && (
                <div className={`p-3 rounded-xl bg-gradient-to-br ${selectedType.color}`}>
                  <selectedType.icon className="h-6 w-6 text-white" />
                </div>
              )}
              <div>
                <h4 className="font-semibold text-white">{watch('name') || 'Business Name'}</h4>
                <p className="text-sm text-gray-400">{selectedType?.label || 'Business Type'}</p>
              </div>
            </div>

            <div className="space-y-3">
              <ReviewRow label="Email" value={user?.email || 'N/A'} />
              <ReviewRow label="Phone" value={watch('phone') || 'Not provided'} />
              <ReviewRow label="Address" value={watch('address') || 'Not provided'} />
              <ReviewRow label="Website" value={watch('website') || 'Not provided'} />
              <ReviewRow 
                label="Sales Channels" 
                value={((preferences.salesChannels as string[]) || []).length > 0 
                  ? `${((preferences.salesChannels as string[]) || []).length} selected`
                  : 'Not specified'} 
              />
              <ReviewRow label="Business Stage" value={preferences.businessMaturity || 'Not specified'} />
              <ReviewRow label="Revenue Range" value={preferences.revenueRange || 'Not specified'} />
              <ReviewRow label="Team Size" value={preferences.employeeCount || 'Not specified'} />
              <ReviewRow label="Payment Methods" value={((preferences.paymentMethods as string[]) || []).length > 0 
            ...
            render={({ field }) => (
              <FormItem className="flex items-start gap-3 p-4 rounded-xl bg-white/5 border border-white/10">
                <FormControl>
                  ...
                  >
                    {field.value && <CheckCircle className="h-5 w-5 text-white" />}
                  </button>
                </FormControl>
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    By continuing, you agree to our terms and conditions for using the platform.
                  </p>
                </div>
              </FormItem>
            )}
  ...
  const ReviewRow = ({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) => (
    <div className="flex justify-between items-center py-2">
      ...
      <span className={`text-sm font-medium ${highlight ? 'text-primary' : 'text-white'}`}>
        {value}
      </span>
    </div>
  );
        ...
        <div className="hidden lg:flex lg:w-1/2 relative flex-col justify-between p-12 xl:p-16 border-r border-white/5">
          ...
          <div className="relative z-10">
            ...
            <div className="flex items-center gap-3 mb-8">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">East Africa's Business Platform</p>
              </div>
            </div>
            ...
            <div className="mt-16 space-y-6">
              ...
              <p className="text-lg text-gray-400 max-w-md leading-relaxed">
                Join thousands of SMEs across East Africa. From Kigali to Nairobi, 
                manage operations, accept mobile money, and grow your customer base.
              </p>

              {/* Feature highlights */}
              <div className="grid grid-cols-2 gap-4 pt-8">
                {[
                  { icon: Smartphone, title: 'Mobile Money', desc: 'M-Pesa, MTN, Airtel, Orange' },
                  { icon: Zap, title: 'Offline POS', desc: 'Work without internet' },
                  { icon: Globe, title: 'E-commerce', desc: 'Online marketplace' },
          ...
          <div className="relative z-10 mt-auto pt-8 border-t border-white/10">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-medium text-white">Trusted by 10,000+ businesses</p>
                <p className="text-xs text-gray-500">Across Rwanda, Kenya, Uganda & Tanzania</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="w-full max-w-2xl relative z-10">
            ...
            <div className="mb-8">
                  ...
                  return (
                    <div key={s.number} className="flex items-center flex-shrink-0">
                      <div className="flex flex-col items-center">
                        ...
                        >
                          {step > s.number ? <CheckCircle className="h-4 w-4 sm:h-5 sm:w-5" /> : <Icon className="h-4 w-4 sm:h-5 sm:w-5" />}
                        </div>
                        <span className={`text-[10px] sm:text-xs mt-1 hidden md:block whitespace-nowrap ${
              ...
              <div className="flex justify-between items-center">
                <span className="text-sm font-medium text-white">Step {step} of {steps.length}</span>
            ...
            <div className="text-center mb-8">
              <h2 className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-white via-gray-200 to-gray-400 bg-clip-text text-transparent mb-2">
                ...
                {step === 5 && (isFreelance ? 'Service Offerings' : 'Product Types')}
                {step === 6 && 'Revenue & Team'}
                {step === 7 && 'Growth Plans'}
                {step === 8 && 'Payment Methods'}
              ...
              <p className="text-gray-400 text-sm">
                ...
                {step === 5 && (isFreelance ? 'How will you structure your services?' : 'What kind of products will you sell?')}
            ...
            <div className="text-center mt-8 space-y-4">
              <p className="text-gray-400 text-sm">
                Need help?{' '}
                <a href="/support" className="font-semibold text-primary hover:text-accent transition-colors underline underline-offset-4">
                  Contact Support
                </a>
              </p>
            </div>
