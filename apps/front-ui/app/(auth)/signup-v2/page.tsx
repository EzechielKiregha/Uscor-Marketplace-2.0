...
const schema = z.object({
  ...
  workerRole: z.enum(['ADMIN', 'STAFF', 'MANAGER', 'FREELANCER', 'SUPERVISOR', 'PRIMARY', 'ASSISTANT']).optional(),
...
export default function SignupPageV2() {
  ...
  const [createBusiness, { loading: businessLoading }] = useMutation(CREATE_BUSINESS);
        ...
        <div className="relative z-10">
          ...
          <div className="mt-16 space-y-6">
            ...
            <p className="text-lg text-gray-400 max-w-md leading-relaxed">
              Join thousands of businesses across East Africa. Manage your operations, 
              grow your customer base, and thrive in the digital economy.
            </p>
            
            {/* Feature highlights */}
            <div className="grid grid-cols-2 gap-4 pt-8">
              {[
                { icon: <SmartphoneIcon className="h-6 w-6" />, title: 'Mobile Money', desc: 'M-Pesa, MTN, Airtel' },
                { icon: <ZapIcon className="h-6 w-6" />, title: 'Offline POS', desc: 'Work without internet' },
                { icon: <GlobeIcon className="h-6 w-6" />, title: 'E-commerce', desc: 'Online marketplace' },
                { icon: <ShieldCheckIcon className="h-6 w-6" />, title: 'Verified', desc: 'KYC protection' },
              ].map((feature, idx) => (
              ...
              {steps.map((s, idx) => (
                <div key={s.number} className="flex items-center">
                  <div className="flex flex-col items-center">
                    ...
                    <span className={`text-xs mt-2 hidden sm:block ${step >= s.number ? 'text-white' : 'text-gray-500'}`}>
                      {s.title}
                    </span>
                  </div>
                  {idx < steps.length - 1 && (
              ...
              {step === 1 && (
                <div className="space-y-4 signin-step">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-gray-300 font-medium">Account Type</FormLabel>
                        <FormControl>
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                            {[
                              { value: 'Client', label: 'Client', icon: <UserIcon className="h-5 w-5" />, desc: 'Browse & shop' },
                              { value: 'Business', label: 'Business', icon: <Building2Icon className="h-5 w-5" />, desc: 'Sell & manage' },
                              { value: 'Worker', label: 'Worker', icon: <BriefcaseIcon className="h-5 w-5" />, desc: 'Work & earn' },
                            ].map((option) => (
                              <button
                                key={option.value}
                                type="button"
                              ...
                              >
                                <div className={`mb-2 ${field.value === option.value ? 'text-primary' : 'text-gray-400 group-hover:text-primary'}`}>
                                  {option.icon}
                                </div>
                                <h3 className={`font-semibold text-sm ${field.value === option.value ? 'text-white' : 'text-gray-300'}`}>
                                  {option.label}
                                </h3>
                ...
                <div className="space-y-4 signin-step">
                  ...
                  {role === 'Business' && !businessTypeFromUrl && (
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-gray-300 font-medium">Business Type</FormLabel>
                          <FormControl>
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-60 overflow-y-auto p-2 bg-white/5 rounded-xl border border-white/10">
                              {businessTypes.map((bt) => (
                                ...
                                >
                                  <span className="text-lg mb-1 block">{bt.icon}</span>
                                  <span className={`text-xs font-medium ${field.value === bt.value ? 'text-white' : 'text-gray-400'}`}>
                                    {bt.label.split('&')[0]}
                                  </span>
                                </button>
                              ))}
                  ...
                  {role === 'Worker' && (
                    <>
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel className="text-gray-300 font-medium">Your Role</FormLabel>
                            <FormControl>
                              <div className="grid grid-cols-2 gap-2 max-h-48 overflow-y-auto p-2 bg-white/5 rounded-xl border border-white/10">
                                {workerRoles.map((wr) => (
                                  ...
                                  >
                                    <span className="text-lg mb-1 block">{wr.icon}</span>
                                    <span className={`text-xs font-medium ${field.value === wr.value ? 'text-white' : 'text-gray-400'}`}>
                                      {wr.label}
                                    </span>
                                  </button>
                                ))}
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel className="text-gray-300 font-medium">Business ID</FormLabel>
                            <FormControl>
                              <input
                                {...field}
                  ...
                  {watch('password') && (
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-gray-500">
                        ...
                        {watch('password').length >= 6 && watch('password').length < 10 && 'Fair'}
                        {watch('password').length >= 10 && watch('password').length < 14 && 'Good'}
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between items-center py-2 border-b border-white/5">
                        ...
                        <span className="text-white font-medium flex items-center gap-2">
                          {role === 'Client' && <UserIcon className="h-4 w-4" />}
                          {role === 'Business' && <Building2Icon className="h-4 w-4" />}
                          {role === 'Worker' && <BriefcaseIcon className="h-4 w-4" />}
                          {role}
                        </span>
                      </div>
                      <div className="flex justify-between items-center py-2 border-b border-white/5">
                        <span className="text-gray-400 text-sm">{role === 'Business' ? 'Business Name' : 'Full Name'}:</span>
                        <span className="text-white font-medium">{watch('fullName')}</span>
                      </div>
                      <div className="flex justify-between items-center py-2 border-b border-white/5">
                        <span className="text-gray-400 text-sm">Email:</span>
                        <span className="text-white font-medium">{watch('email')}</span>
                      </div>
                      <div className="flex justify-between items-center py-2 border-b border-white/5">
                      ...
                      {watch('phone') && (
                        <div className="flex justify-between items-center py-2 border-b border-white/5">
                          ...
                          <span className="text-white font-medium">{watch('phone')}</span>
                        </div>
                      )}
                      {role === 'Business' && watch('businessType') && (
                        <div className="flex justify-between items-center py-2 border-b border-white/5">
                          ...
                          <span className="text-white font-medium">
                            {businessTypes.find(bt => bt.value === watch('businessType'))?.label || watch('businessType')}
                          </span>
                        </div>
                      )}
                      {role === 'Worker' && watch('workerRole') && (
                        <div className="flex justify-between items-center py-2 border-b border-white/5">
                          <span className="text-gray-400 text-sm">Worker Role:</span>
                          <span className="text-white font-medium">
                            {workerRoles.find(wr => wr.value === watch('workerRole'))?.label || watch('workerRole')}
                          </span>
                        </div>
                      )}
                      {role === 'Worker' && watch('businessId') && (
                        <div className="flex justify-between items-center py-2 border-b border-white/5">
                          ...
                          <span className="text-white font-medium">{watch('businessId')}</span>
                        </div>
                      )}
                    </div>
                  </div>
