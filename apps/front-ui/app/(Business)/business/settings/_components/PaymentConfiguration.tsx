...
function PaymentCode({
	...
	const [paymentCode, setPaymentCode] = useState<string | null | undefined>(
		null,
	);
	...
	useEffect(() => {
		...
		setPaymentCode(code);
	}, [productOwnerCodes, country, provider, amount, generatePaymentCode]);

	return (
		<div className="bg-muted p-3 rounded-lg">
			<div className="flex justify-between items-center">
				<span className="font-mono text-sm">
					{paymentCode || "Generating code..."}
				</span>
				{paymentCode && (
					<Button
						variant="ghost"
	...
	function generatePaymentCode(
		...
		switch (provider) {
			...
			case "ORANGE_MONEY":
				...
				if (
					["RWANDA", "UGANDA", "KENYA", "TANZANIA", "BURUNDI"].includes(country)
				)
...
export default function PaymentConfiguration({}: PaymentConfigurationProps) {
	...
	useEffect(() => {
		...
		businessData.paymentConfig.orangeCode,
		businessData.paymentConfig.mpesaCode,
		businessData.paymentConfig.airtelCode,
		businessData.paymentConfig.mtnCode,
		businessData?.paymentConfig,
	]);
									...
									<div className="space-y-4">
										...
										<div className="flex items-center gap-2">
											<Input
												...
												value={testAmount}
												onChange={(e) =>
													setTestAmount(parseInt(e.target.value, 10) || 1000)
												}
												className="w-24"
												min="1"
											/>
					...
					<div>
						...
						<div className="pl-2 border-l border-border space-y-4">
							<div>
								...
								/>
								<p className="mt-1 text-xs text-muted-foreground">
									Provide your bank account details for customers who prefer
									bank transfers
								</p>
							</div>
						</div>
					...
					<div className="border border-orange-400/60 dark:border-orange-500/70 rounded-lg p-4 bg-muted">
							...
							{businessType === "ARTISAN" && (
								<>
									<li className="flex items-start gap-2">
										...
										<span>
											Display your mobile money codes prominently at your
											workshop or store
										</span>
									</li>
									<li className="flex items-start gap-2">
										<CheckCircle className="h-4 w-4 text-success mt-1 flex-shrink-0" />
										<span>
											For custom orders, collect a deposit via mobile money
									...
									<li className="flex items-start gap-2">
										<CheckCircle className="h-4 w-4 text-success mt-1 flex-shrink-0" />
										<span>
											Use the payment code preview to train your staff on
									...
									<li className="flex items-start gap-2">
										<CheckCircle className="h-4 w-4 text-success mt-1 flex-shrink-0" />
										<span>
											For wood workers and artisans, mobile money is preferred
											for quick, secure transactions
										</span>
									</li>
								</>
							)}
							{businessType === "GROCERY" && (
								<>
									<li className="flex items-start gap-2">
										<CheckCircle className="h-4 w-4 text-success mt-1 flex-shrink-0" />
										<span>
											Set up multiple mobile money options to accommodate all
									...
									<li className="flex items-start gap-2">
										<CheckCircle className="h-4 w-4 text-success mt-1 flex-shrink-0" />
										<span>
											Display payment codes at checkout counters for quick
											reference
										</span>
									</li>
									<li className="flex items-start gap-2">
										<CheckCircle className="h-4 w-4 text-success mt-1 flex-shrink-0" />
										<span>
											For delivery orders, include payment code instructions in
											order confirmations
										</span>
									</li>
									<li className="flex items-start gap-2">
										<CheckCircle className="h-4 w-4 text-success mt-1 flex-shrink-0" />
										<span>
											Train all staff on verifying mobile money payments before
											handing over goods
										</span>
									</li>
								</>
							...
							{businessType === "CAFE" && (
								<>
									<li className="flex items-start gap-2">
										<CheckCircle className="h-4 w-4 text-success mt-1 flex-shrink-0" />
										<span>
											Use mobile money for quick transactions during peak hours
										</span>
									</li>
									<li className="flex items-start gap-2">
										<CheckCircle className="h-4 w-4 text-success mt-1 flex-shrink-0" />
										<span>
											For catering orders, collect partial payment via mobile
											money to secure bookings
										</span>
									</li>
									<li className="flex items-start gap-2">
										<CheckCircle className="h-4 w-4 text-success mt-1 flex-shrink-0" />
										<span>
											Display payment codes on table tents for easy customer
											reference
										</span>
									</li>
									<li className="flex items-start gap-2">
										<CheckCircle className="h-4 w-4 text-success mt-1 flex-shrink-0" />
										<span>
											Integrate payment verification into your POS workflow to
											reduce errors
										</span>
									</li>
								</>
							...
							{businessType === "RETAIL" && (
								<>
									<li className="flex items-start gap-2">
										<CheckCircle className="h-4 w-4 text-success mt-1 flex-shrink-0" />
										<span>
											Offer mobile money payment as the default option to speed
											up checkout
										</span>
									</li>
									<li className="flex items-start gap-2">
										<CheckCircle className="h-4 w-4 text-success mt-1 flex-shrink-0" />
										<span>
											For high-value items, use mobile money for secure,
									...
									<li className="flex items-start gap-2">
										<CheckCircle className="h-4 w-4 text-success mt-1 flex-shrink-0" />
										<span>
											Train staff to verify payment confirmation before
											completing sales
										</span>
									</li>
									<li className="flex items-start gap-2">
										<CheckCircle className="h-4 w-4 text-success mt-1 flex-shrink-0" />
										<span>
											Display your payment codes at multiple points in your
						...
						<div className="mt-4 p-3 bg-background rounded-lg border border-orange-400/60 dark:border-orange-500/70">
							<p className="text-sm">
								...
								your payment codes are correctly configured to avoid lost sales
					...
					<div className="flex justify-end gap-3 pt-4 border-t border-border">
						<Button
							...
							onClick={() => {
								...
								setFormData({
									mobileMoneyEnabled:
										businessData?.paymentConfig?.mobileMoneyEnabled || true,
									mtnCode: businessData?.paymentConfig?.mtnCode || "",
									airtelCode: businessData?.paymentConfig?.airtelCode || "",
									orangeCode: businessData?.paymentConfig?.orangeCode || "",
									mpesaCode: businessData?.paymentConfig?.mpesaCode || "",
