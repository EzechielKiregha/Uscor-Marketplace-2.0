...
function CategoryPopover() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const catName =
        (form.elements.namedItem("catName") as HTMLInputElement)?.value || "";
      const catDesc =
        (form.elements.namedItem("catDesc") as HTMLInputElement)?.value || "";
      if (!catName.trim()) {
    ...
    <Dialog>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Create your product category here. Click save when you&apos;re done.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="grid gap-4">
            ...
            <div className="grid gap-3">
              ...
              <Input
                id="catDesc"
                name="catDesc"
                defaultValue="Electronics products and accessories"
              />
            </div>
          </div>
          <DialogFooter>
...
export default function ProductForm({
  ...
  const [imageFile, setImageFile] = useState<File | string>(
    initialData?.medias[0] ? initialData.medias[0].url : "",
  );
  const [isSubmitting, setIsSubmitting] = useState(false);
  const { showToast } = useToast();
  ...
  }, [storesData, catData, selectedStoreId, selectedCatId]);
    ...
    try {
      ...
      if (imageFile) {
        ...
        if (imageFile instanceof File) {
          blob = await put(
            `business/products/medias/${Date.now()}-${imageFile}`,
            imageFile,
            {
              access: "public",
              token: blobToken,
          ...
          await updateProduct({
            variables: {
              ...
              mediaInput: {
                ...
                pathname:
                  blob.pathname || (imageFile as string).split(".com/")[1],
          ...
          {createCategoryOpen && catData.categories.length !== 0 && (
            <>
              <p className="text-sm text-muted-foreground mt-1">
                or create a category for this product
              </p>
              <CategoryPopover />
            </>
          )}
