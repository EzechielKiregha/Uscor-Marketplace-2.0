...
export const useSales = (storeId: string, wUserId: string, role?: string) => {
	...
	const createSale = useCallback(
		async (workerId?: string, clientId?: string) => {
			try {
		...
		[storeId, createSaleMutation, showToast, router.refresh],
	);

	// Add product to current sale
	const addProductToSale = useCallback(
		async (productId: string, quantity: number = 1, modifiers?: any) => {
			if (!currentSaleId) {
	...
	const completeSale = useCallback(
		...
		[currentSaleId, completeSaleMutation, showToast],
	);
