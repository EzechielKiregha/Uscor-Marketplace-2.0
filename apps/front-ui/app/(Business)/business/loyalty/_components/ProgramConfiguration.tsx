...
export default function ProgramConfiguration({
  ...
  const [formData, setFormData] = useState({
    ...
    description:
      "Earn points for every purchase and redeem for discounts or free local products",
    pointsPerPurchase: 1,
  ...
  const validateForm = () => {
    ...
    if (formData.minimumPointsToRedeem <= formData.pointsPerPurchase) {
      errors.minimumPointsToRedeem =
        "Minimum points should be higher than points per purchase";
    }
  ...
  const handleInputChange = (
    ...
    setFormData((prev) => ({
      ...prev,
      [name]:
        name === "pointsPerPurchase" || name === "minimumPointsToRedeem"
          ? Number(value)
          : value,
    }));
  ...
  const calculateRedemptionValue = () => {
    return (
      formData.minimumPointsToRedeem / formData.pointsPerPurchase
    ).toFixed(2);
  };
          ...
          <div className="border border-border hover:border-primary hover:bg-primary/5 rounded-lg p-4 bg-muted">
            ...
            <div className="mt-4 p-3 bg-background rounded-lg border border-border hover:border-primary hover:bg-primary/5">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                ...
                Collect {formData.minimumPointsToRedeem} points to get $
                {calculateRedemptionValue()} off your next purchase."
              </p>
            </div>
          </div>
          ...
          <div className="border border-border hover:border-primary hover:bg-primary/5 rounded-lg p-4">
            ...
            <ul className="space-y-2">
              <li className="flex items-start gap-2">
                ...
                <span>
                  Encourage repeat customers from your local community
                </span>
              </li>
              <li className="flex items-start gap-2">
                <CheckCircle className="h-4 w-4 text-success mt-1 flex-shrink-0" />
                <span>
                  Perfect for artisans, wood workers, and local craftsmen to
              ...
              <li className="flex items-start gap-2">
                <CheckCircle className="h-4 w-4 text-success mt-1 flex-shrink-0" />
                <span>
                  Rewards customers while helping them access more affordable
                  local products
                </span>
              </li>
              <li className="flex items-start gap-2">
                <CheckCircle className="h-4 w-4 text-success mt-1 flex-shrink-0" />
                <span>
                  Create a community around your locally-made products and
                  services
                </span>
              </li>
            </ul>
            ...
            <div className="mt-4 p-3 bg-muted rounded-lg">
              <p className="text-sm">
                <strong>Pro Tip:</strong> For local artisans and craftsmen,
                consider offering special redemption options like "100 points =
                10% off a handmade product" to highlight the value of your
                locally-made goods.
              </p>
            </div>
          </div>
