...
export default function RedemptionProcess({
	...
	const filteredCustomers = useMemo(() => {
		...
		return customersData.customerPoints.filter(
			(customer: any) =>
	...
	const handleRedeemPoints = async () => {
		if (
			!selectedCustomer ||
			pointsToRedeem <= 0 ||
			pointsToRedeem > selectedCustomer.totalPoints
		)
			return;
		...
		try {
			...
			showToast(
				"success",
				"Points Redeemed",
				`${pointsToRedeem} points redeemed for ${selectedCustomer.clientName}`,
			);
	...
	const handleScan = () => {
		...
		setTimeout(() => {
			...
			const customer = filteredCustomers.find(
				(c: any) => c.clientId === randomCustomer.id,
			);
			if (customer) {
										...
										{filteredCustomers.slice(0, 5).map((customer: any) => (
											<div
												...
												onClick={() => {
													setSelectedCustomer(customer);
													setPointsToRedeem(
														Math.min(100, customer.totalPoints),
													);
												}}
											>
									...
									<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
										<div>
											...
											<p className="mt-1 text-sm text-muted-foreground">
												Customer will earn {transactionAmount} points for this
												transaction
											</p>
										</div>
					...
					<CardContent className="space-y-6">
							...
							<div className="space-y-3">
								<div>
									<h4 className="font-medium mb-1">
										For Wood Workers & Tool Makers
									</h4>
									<p className="text-sm text-muted-foreground">
										Offer special redemption options like "200 points = 10% off
								...
								<div>
									...
									<p className="text-sm text-muted-foreground">
										Create seasonal promotions where points earned on local
										products can be redeemed for community events
									</p>
								</div>
								...
								<div>
									...
									<p className="text-sm text-muted-foreground">
										Partner with other local businesses to create a
										community-wide loyalty program
									</p>
								</div>
							</div>
						</div>
						...
						<div className="border hover:border-primary  rounded-lg p-4">
							...
							<div className="space-y-2 mt-3">
								...
								<div className="p-2 bg-background rounded border">
									<p className="text-sm">
										500 points = Free small handmade item
									</p>
								</div>
								<div className="p-2 bg-background rounded border">
								...
								<div className="p-2 bg-background rounded border">
									<p className="text-sm">
										2000 points = Featured on local artisan showcase
									</p>
								</div>
							</div>
