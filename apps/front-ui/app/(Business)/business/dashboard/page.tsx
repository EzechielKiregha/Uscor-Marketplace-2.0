...
export default function BusinessDashboardPage() {
  ...
  useEffect(() => {
    if (storesData?.stores && businessData?.businessDashboard) {
      ...
      storesData.stores.forEach((store: StoreEntity) => {
        ...
        const storeRevenue =
          store.sales?.reduce(
            (sum: number, sale: SaleEntity) => sum + (sale.totalAmount || 0),
            0,
          ) || 0;
        ...
        const storeLowStock =
          store.products?.filter(
            (product: ProductEntity) => product.quantity < product.minQuantity,
          ).length || 0;
  ...
  const _salesData = businessData.businessDashboard.salesData;
      ...
      <div className="flex flex-col justify-between sm:flex-row gap-3 w-full sm:w-auto">
        ...
        {/* <Button
          ...
          onClick={() => {
            ...
            showToast(
              "success",
              "Redirecting to new sale page",
              "You are now redirecting to the point of sale page",
              false,
              9000,
            );
          }}
