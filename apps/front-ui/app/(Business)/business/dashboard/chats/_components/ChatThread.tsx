				...
				<div className="flex items-center gap-3">
					...
					<div className="min-w-0 flex-1">
						<h2 className="font-semibold text-sm md:text-base truncate">
							{data.chat.client ? data.chat.client.fullName : "Anonymous"}
						</h2>
						{data.chat.product && (
							<p className="text-xs md:text-sm text-muted-foreground truncate">
								Product: {data.chat.product.title}
							</p>
						)}
					</div>
				</div>
				...
				<div className="flex items-end gap-2">
					...
					<div className="flex-1 relative">
						<textarea
							...
							className="w-full p-2 md:p-3 border hover:border-primary  rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-primary/50 min-h-[40px] max-h-[120px] text-sm md:text-base"
							rows={1}
						/>
					</div>
					<Button
