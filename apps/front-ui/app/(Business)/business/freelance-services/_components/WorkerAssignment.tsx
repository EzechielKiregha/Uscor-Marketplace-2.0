...
import { ASSIGN_WORKER_TO_SERVICE } from "@/graphql/freelance.gql";
...
export default function WorkerAssignment({
  ...
  const filteredWorkers = useMemo(() => {
    ...
    return workersData?.workers?.filter(
      (worker: WorkerEntity) =>
        (worker.fullName ? worker.fullName : "Unknown Worker")
          .toLowerCase()
  ...
  const assignedWorkers = useMemo(() => {
    ...
    return workersData?.workers?.filter((worker: WorkerEntity) =>
      ...
      (worker.workerServiceAssignments ?? []).some(
        (assignment) => assignment.freelanceServiceId === serviceId,
      ),
    );
              ...
              <tbody>
                {assignedWorkers.map((worker: WorkerEntity) => {
                  ...
                  const assignment = (
                    ...
                    (a: any) =>
                      (a.freelanceServiceId ?? a.serviceId) === serviceId,
                  );
                  return (
                    <tr
                      key={worker.id}
                    ...
                    >
                      ...
                      <td className="py-3">
                        {new Date(
                          assignment ? assignment.assignedAt : new Date(),
                        ).toLocaleDateString()}
                      </td>
                      <td className="py-3">
                        <Button
              ...
              {!selectedWorker ? (
                  ...
                  {filteredWorkers.length === 0 ? (
                    ...
                    <div className="max-h-60 overflow-y-auto">
                      {filteredWorkers.map((worker: WorkerEntity) => (
                        <div
                          key={worker.id}
                          className="flex items-center justify-between p-3 border hover:border-primary  rounded-lg hover:bg-muted/50 cursor-pointer mb-2"
                ...
                <div className="space-y-6">
                  ...
                  <div className="p-4 bg-muted rounded-lg">
                    ...
                    <p className="text-sm">
                      {selectedWorker.fullName} will be assigned as a{" "}
                      {role.toLowerCase()} for this service. They will be able
                      to manage service orders and interact with customers.
                    </p>
                  </div>
