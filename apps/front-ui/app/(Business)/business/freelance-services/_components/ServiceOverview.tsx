					...
					<CardContent>
						<div className="space-y-4">
							...
							<div className="p-3 bg-muted rounded-lg">
								...
								<p className="text-2xl font-bold">
									${calculateStats.revenue.toFixed(2)}
								</p>
				...
				<Card className="border border-orange-400/60 dark:border-orange-500/70 bg-card">
					...
					<CardContent>
						<div className="space-y-3">
							...
							{[...services]
								.sort((a, b) => {
									const aOrders = serviceOrders.filter(
										(o) => o.serviceId === a.id,
									).length;
									const bOrders = serviceOrders.filter(
										(o) => o.serviceId === b.id,
									).length;
								...
								.slice(0, 5)
								.map((service) => {
									const serviceOrdersCount = serviceOrders.filter(
										...
										<p className="text-2xl font-bold truncate">
											{
												services
													.map((service) => ({
														...service,
														orderCount: serviceOrders.filter(
															(o) => o.serviceId === service.id,
														).length,
													}))
										...
										<p className="text-2xl font-bold truncate">
											{
												services
													.map((service) => {
														const serviceOrderss = serviceOrders.filter(
															(o) => o.serviceId === service.id,
														);
														const completed = serviceOrderss.filter(
															(o) => o.status === "COMPLETED",
														).length;
														const rate =
															serviceOrderss.length > 0
																? (completed / serviceOrderss.length) * 100
																: 0;
