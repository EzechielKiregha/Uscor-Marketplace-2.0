...
import type { BusinessEntity, FreelanceServiceEntity } from "@/lib/types";
...
export default function ServiceManagement({
	...
	const handleDelete = async () => {
		if (
			confirm(
				"Are you sure you want to delete this service? This will also delete all associated orders.",
			)
		) {
			try {
				await deleteService({
	...
	const rawType = ((user as BusinessEntity)?.businessType || "")
		.toString()
		.toLowerCase();
	const typeKey = Object.keys(businessBenefitsMap).includes(rawType)
		? rawType
		: "artisan";
					...
					<div className="border border-orange-400/60 dark:border-orange-500/70 rounded-lg p-4">
						...
						{benefits.tip && (
							<div className="mt-4 p-3 bg-muted rounded-lg">
								<p className="text-sm">
									<strong>Pro Tip:</strong> {benefits.tip} Uscor makes these
									services easy to customize, assign staff, set availability,
									and track orders so you can focus on delivering great
									experiences.
								</p>
							</div>
						)}
