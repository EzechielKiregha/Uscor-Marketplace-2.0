...
import { DELETE_STORE, GET_STORES } from "@/graphql/store.gql";
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {data.stores.map((store: StoreEntity) => {
          ...
          return (
            <div
              key={store.id}
              className="border border-orange-400/60 dark:border-orange-500/70 rounded-lg overflow-hidden bg-card"
            >
              <div className="p-4">
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  ...
                  <div className="flex items-center gap-2 p-2 bg-muted rounded">
                    <AlertTriangle
                      className={`h-4 w-4 ${lowStockProducts > 0 ? "text-red-500" : "text-gray-400"}`}
                    />
                    <div>
                      <p className="text-xs text-muted-foreground">Low Stock</p>
                      <p
                        className={`font-semibold text-sm ${lowStockProducts > 0 ? "text-red-500" : ""}`}
                      >
                        {lowStockProducts}
                      </p>
                    </div>
