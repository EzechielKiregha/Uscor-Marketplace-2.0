		...
		<div className="space-y-6">
			...
			{showForm && (
				<div className="bg-card border border-border rounded-lg p-4 space-y-4">
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
						...
						<div>
							<label className="text-sm text-muted-foreground mb-1 block">Min Qty</label>
							<Input
								type="number"
								placeholder="e.g. 10"
						...
						<div>
							<label className="text-sm text-muted-foreground mb-1 block">Max Qty (optional)</label>
							<Input
								type="number"
								placeholder="e.g. 99"
			...
			{Object.keys(grouped).length === 0 ? (
				<EmptyState
					...
					actionLabel="Add Price Tier"
					onAction={() => setShowForm(true)}
				/>
			) : (
				<MotionStagger className="space-y-4">
					{Object.entries(grouped).map(([productTitle, group]: [string, any]) => (
									...
									<div className="flex items-center gap-3">
										...
										<div>
											...
											<p className="text-xs text-muted-foreground">
												Retail: ${group.product?.price?.toFixed(2)} &middot; Stock:{" "}
												{group.product?.stock}
											</p>
										</div>
									</div>
									...
									{group.tiers
										...
										.map((tier: any) => {
											const discount = group.product?.price
												? Math.round(
														((group.product.price - tier.price) / group.product.price) * 100,
													)
												: 0;
											return (
												<div
