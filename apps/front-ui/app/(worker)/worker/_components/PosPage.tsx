...
export default function PosPage({ selectedStoreId }: PosPageProps) {
  ...
  const handleAddProduct = async (product: any, quantity: number = 1) => {
    ...
    if (!isOnline) {
      ...
      showToast('info', 'Offline Mode', 'Product added to local sale. Will sync when online.');
    } else {
      try {
  ...
  const handleRemoveProduct = async (productId: string) => {
    if (!isOnline) {
      ...
      const updatedSale = {
        ...currentSale,
        saleProducts: currentSale.saleProducts.filter((sp: any) => sp.productId !== productId),
      ...
      showToast('info', 'Offline Mode', 'Product removed from local sale. Will sync when online.');
    } else {
      try {
  ...
  const handleCompleteSale = async () => {
    ...
    if (!isOnline) {
      ...
      showToast('info', 'Offline Mode', 'Sale completed offline. Will sync when online.');
      setCurrentSale(null);
  ...
  const handleMobileMoneyPayment = async () => {
    ...
    try {
      ...
      showToast('info', 'Payment Instruction', `Dial: ${data.processMobileMoneyPayment.ussdCode}`);
    } catch (error: any) {
      showToast('error', 'Payment Error', error.message || 'Failed to initiate mobile payment');
    }
  };
  ...
  const filteredInventory = inventoryData?.workerInventory?.items?.filter((item: any) => 
        ...
        <div className="bg-warning/10 border border-warning/30 rounded-lg p-3 flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Sales will sync when connection is restored. Currently saving to local storage.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-card border border-border rounded-lg overflow-hidden">
            ...
            <div className="p-4">
              {currentSale?.saleProducts && currentSale.saleProducts.length > 0 ? (
                ...
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm mt-1">Search and select products to add</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="space-y-3">
            <Button 
              ...
              disabled={!paymentMethod || !currentSale?.saleProducts || currentSale.saleProducts.length === 0}
            ...
            <div className="p-6">
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <p className="text-primary font-bold text-lg">${selectedProduct.price.toFixed(2)}</p>
                </div>
                <Button
                  variant="ghost"
                  size="icon"
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  ...
                  <p className="font-medium">{selectedProduct.stock} units available</p>
                </div>
              </div>
