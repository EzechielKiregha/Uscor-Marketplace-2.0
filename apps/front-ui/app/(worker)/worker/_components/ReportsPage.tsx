...
export default function ReportsPage({ selectedStoreId }: ReportsPageProps) {
  ...
  const [reportType, setReportType] = useState<'sales' | 'inventory' | 'performance'>('sales');
  ...
  } = useQuery(GET_WORKER_PERFORMANCE, {
    variables: { 
      workerId: user?.id,
  ...
  } = useQuery(GET_WORKER_SALES_HISTORY, {
    variables: { 
      workerId: user?.id,
      storeId: selectedStoreId,
      timeRange
  ...
  if (reportsLoading || performanceLoading || salesHistoryLoading) return <Loader loading={true} />;
        ...
        <div className="bg-card border border-border rounded-lg p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold">
                ${(reportData?.totalSales / reportData?.totalOrders || 100.40).toFixed(2)}
              </p>
            </div>
          </div>
        </div>
              ...
              <tbody>
                {reportData?.topSellingProducts?.map((product: any) => (
                  <tr key={product.id} className="border-b border-border hover:bg-muted/50">
                    ...
                    <td className="py-3 px-4">{product.profitMargin.toFixed(1)}%</td>
                    <td className="py-3 px-4">
        ...
        <div className="p-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="border border-border rounded-lg p-4">
              ...
              <p className="text-2xl font-bold">{performanceData?.shiftsCompleted || 24}</p>
              <p className="text-sm text-muted-foreground">Shifts completed this {timeRange}</p>
            </div>
            
            <div className="border border-border rounded-lg p-4">
              <h3 className="font-medium mb-2">Sales Contribution</h3>
              <p className="text-2xl font-bold">${performanceData?.personalSales?.toFixed(2) || '3,240.00'}</p>
              <p className="text-sm text-muted-foreground">Sales processed by you</p>
            </div>
            
            <div className="border border-border rounded-lg p-4">
              <h3 className="font-medium mb-2">Customer Satisfaction</h3>
              <p className="text-2xl font-bold">{performanceData?.customerSatisfaction?.toFixed(1) || '4.7'}</p>
