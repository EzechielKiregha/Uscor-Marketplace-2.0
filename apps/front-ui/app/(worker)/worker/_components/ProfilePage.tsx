...
export default function ProfilePage({}: ProfilePageProps) {
    ...
    try {
      ...
      if (avatarFile) {
        ...
        if (avatarFile instanceof File) {
          const blob = await put(`business/medias/${Date.now()}-${avatarFile.name}`, avatarFile, {
            access: 'public',
            token: blobToken,
          });
          avatarUrl = blob.url;
  ...
  if (workerError) return <div>Error loading profile: {workerError.message}</div>;
          ...
          <div className="bg-card border border-border rounded-lg overflow-hidden">
            <div className="p-6 text-center">
              ...
              {isEditing ? (
                ...
                <div className="mt-4">
                  <h2 className="text-xl font-bold">{worker?.fullName || 'Worker'}</h2>
                  <p className="text-muted-foreground">{worker?.role || 'Staff'}</p>
              ...
              <form onSubmit={handleSubmit} className="p-6">
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label htmlFor="fullName" className="text-sm font-medium mb-1 flex items-center gap-2">
              ...
              <div className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <p className="text-muted-foreground">{worker?.fullName || 'Not provided'}</p>
                  </div>
                  
                  <div>
                    <h3 className="font-medium mb-1">Email Address</h3>
                    <p className="text-muted-foreground">{worker?.email || 'Not provided'}</p>
                  </div>
                  
                  <div>
                    <h3 className="font-medium mb-1">Phone Number</h3>
                    <p className="text-muted-foreground">{worker?.phone || 'Not provided'}</p>
                  </div>
            ...
            <div className="p-4">
              {recentShifts.length === 0 ? (
                <div className="text-center py-4 text-muted-foreground">
                  ...
                  <p className="text-sm mt-1">Your shift history will appear here</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {recentShifts.map((shift: any) => (
                    <div key={shift.id} className="flex items-center justify-between p-3 border border-border rounded-lg">
                      <div>
                        <p className="font-medium">
                        ...
                        <p className="text-sm text-muted-foreground">
                          ...
                          {shift.endTime ? new Date(shift.endTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'In Progress'}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="font-bold">${shift.sales?.toFixed(2) || '0.00'}</p>
                        <p className="text-xs text-muted-foreground">{shift.transactions || 0} sales</p>
                      </div>
                    </div>
                  ))}
                </div>
