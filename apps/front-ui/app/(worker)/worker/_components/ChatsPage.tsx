...
export default function ChatsPage({}: ChatsPageProps) {
  const { user } = useMe();
  const [selectedChat, setSelectedChat] = useState<TypedChatEntity | null>(null);
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'ACTIVE':
        return <span className="px-2 py-1 bg-success/10 text-success text-xs rounded-full">Active</span>;
      case 'CLOSED':
        return <span className="px-2 py-1 bg-muted text-muted-foreground text-xs rounded-full">Closed</span>;
      default:
        return <span className="px-2 py-1 bg-muted text-muted-foreground text-xs rounded-full">{status}</span>;
    }
  };

  const chatParticipants: ChatParticipantEntity[] = ((chatsData?.chats?.items || []) as ChatEntity[])
    .flatMap((chat) => chat.participants || []);

  const typedChats = ((chatsData?.chats?.items || []) as ChatEntity[]).map((chat) => {
    const clientParticipant = chat.participants?.find((participant) => participant.client)?.client;
    const businessParticipant = chat.participants?.find((participant) => participant.business)?.business;
    ...
    return ({
      ...
      negotiationType: (chat.negotiationType as ChatEntity['negotiationType']) || (chat as any).negotiationType,
    });
  }) as TypedChatEntity[];
  ...
  const filteredChats = typedChats.filter((chat) => {
    ...
    const matchesSearch = !searchQuery ||
      participant?.client?.fullName?.toLowerCase().includes(searchQuery.toLowerCase()) ||
      participant?.business?.name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
      chat.messages?.some((msg) => ((msg.content || msg.message) ?? '').toLowerCase().includes(searchQuery.toLowerCase()));
                    ...
                    <div className="flex items-start gap-3">
                      {chat.client?.avatar ? (
                        ...
                        <div className="w-10 h-10 rounded-full bg-muted flex items-center justify-center text-sm">
                          {chat.client?.fullName?.charAt(0) || chat.business?.name?.charAt(0) || 'U'}
                        </div>
                      )}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start">
                          <div>
                            <h3 className="font-medium truncate">
                              {chat.client?.fullName || chat.business?.name || 'Unknown'}
                            </h3>
                            <p className="text-xs text-muted-foreground mt-1">
                              {chat.negotiationType === 'REOWNERSHIP' && <span className="bg-warning/10 text-warning px-1.5 py-0.5 rounded text-xs">Reownership</span>}
                              {chat.negotiationType === 'FREELANCE_ORDER' && <span className="bg-success/10 text-success px-1.5 py-0.5 rounded text-xs">Freelance</span>}
                              {chat.negotiationType === 'PURCHASE' && <span className="bg-primary/10 text-primary px-1.5 py-0.5 rounded text-xs">Purchase</span>}
                              {chat.negotiationType === 'GENERAL' && <span className="bg-muted px-1.5 py-0.5 rounded text-xs">General</span>}
                            </p>
                          </div>
                        ...
                        {lastMessage && (
                          <p className="text-sm text-muted-foreground truncate mt-1 line-clamp-1">
                            {lastMessage.senderType === 'WORKER' ? 'You: ' : ''}{lastMessage.content}
                          </p>
                        )}
          ...
          {selectedChat ? (
            <div className="bg-card border border-border rounded-lg overflow-hidden flex flex-col h-150">
              ...
              <div className="p-4 bg-muted border-b border-border flex items-center justify-between">
                <div className="flex items-center gap-3">
                  {selectedChat.client?.avatar ? (
                    ...
                    <div className="w-10 h-10 rounded-full bg-muted flex items-center justify-center text-sm">
                      {selectedChat.client?.fullName?.charAt(0) || selectedChat.business?.name?.charAt(0) || 'U'}
                    </div>
                  )}
                  
                  <div>
                    <h3 className="font-medium">
                      {selectedChat.client?.fullName || selectedChat.business?.name}
                    </h3>
                    <p className="text-xs text-muted-foreground">
                      {selectedChat.client?.email || selectedChat.business?.email}
                    </p>
                  </div>
                  ...
                  {selectedChat.product && (
                    <div className="ml-4 flex items-center gap-2 bg-muted/50 rounded-lg px-3 py-1">
                      ...
                      <span className="text-sm">{selectedChat.product.title}</span>
                    </div>
                  )}
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-muted/10">
                {messagesData?.chatMessages?.items?.map((message: ChatMessageWithContentEntity) => (
                  <div 
                    key={message.id} 
                    className={`flex ${
                      message.senderType === 'WORKER' ? 'justify-end' : 'justify-start'
                    }`}
                  >
                    <div className={`max-w-[80%] ${
                      ...
                      <div className="px-3 pb-2 text-xs opacity-70">
                        ...
                        {message.senderType === 'WORKER' && message.isRead && (
                          <span className="ml-1"><CheckCircle className="h-3 w-3 inline" /></span>
                        )}
                      </div>
                    </div>
                  </div>
