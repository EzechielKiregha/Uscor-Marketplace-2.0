// app/worker/shifts/page.tsx (Updated)
'use client';
...
export default function ShiftsPage({ selectedStoreId }: ShiftsPageProps) {
  ...
  const { 
    ...
    variables: { 
      workerId: user?.id,
      storeId: selectedStoreId,
      startDate: timeRange === 'today' ? new Date().toISOString() : 
  ...
  const handleStartShift = async () => {
    if (!isOnline) {
      ...
      showToast('info', 'Offline Mode', 'Shift started. Will sync when online.');
    } else {
      try {
        ...
        showToast('success', 'Shift Started', 'Your shift has been recorded successfully');
        refetchCurrentShift();
      } catch (error: any) {
  ...
  const totalSales = shifts.reduce((sum: number, shift: any) => sum + (shift.sales || 0), 0);
          ...
          {currentShift ? (
            <div className="text-center">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div className="border border-border rounded-lg p-4">
                  ...
                  <p className="text-2xl font-bold">${currentShift.sales?.toFixed(2) || '0.00'}</p>
                </div>
            ...
            <div className="text-center">
              ...
              <p className="text-muted-foreground mb-6">
                Click the button below to start your shift and begin tracking your work hours
              </p>
        ...
        <div className="p-4">
          {shifts.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p className="text-sm mt-1">Your shift records will appear here after you complete shifts</p>
            </div>
          ) : (
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground mt-1">
                        {new Date(shift.startTime).toLocaleTimeString()} - {shift.endTime ? new Date(shift.endTime).toLocaleTimeString() : 'In Progress'}
                      </p>
                    </div>
                  ...
                  {shift.endTime && (
                    <div className="mt-3 flex items-center gap-4 text-sm">
                      <div className="flex items-center gap-1">
                        ...
                        <span>
                          {Math.floor((new Date(shift.endTime).getTime() - new Date(shift.startTime).getTime()) / (1000 * 60 * 60))}h
                        </span>
                      </div>
                      <div className="flex items-center gap-1">
          ...
          <div className="p-4">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="border border-border rounded-lg p-4">
                <div className="flex items-center gap-2 mb-1">
                  <TrendingUp className="h-4 w-4 text-primary" />
                  <span className="text-sm text-muted-foreground">Avg. Sales</span>
                </div>
