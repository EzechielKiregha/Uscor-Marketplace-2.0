...
export default function UserManagement() {
  ...
  const getUserTypeData = () => {
    if (filters.userType === "BUSINESS") {
      ...
      return {
        items: usersData?.all_workers?.items || [],
        total: usersData?.all_workers?.total || 0,
      };
      ...
      // ALL user types
      const allItems = [
        ...(usersData?.all_businesses?.items?.map((item: any) => ({
          ...item,
          userType: "BUSINESS",
        })) || []),
        ...(usersData?.all_clients?.items?.map((item: any) => ({
          ...item,
          userType: "CLIENT",
        })) || []),
        ...(usersData?.all_workers?.items?.map((item: any) => ({
          ...item,
          userType: "WORKER",
        })) || []),
        ...(usersData?.all_admins?.items?.map((item: any) => ({
          ...item,
          userType: "ADMIN",
        })) || []),
      ];
      ...
      return {
        items: allItems,
        total:
          (usersData?.all_businesses?.total || 0) +
  ...
  if (usersLoading) return <TableSkeleton />;
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {(filters.userType === "BUSINESS" ||
            ...
            <select
              ...
              className="w-full sm:w-40 p-2 border hover:border-primary  rounded-lg bg-background focus:outline-none focus:ring-2 focus:ring-primary/50"
            >
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          <span>
                            ...
                            {user.businessType === "BOOKSTORE" &&
                              "📚 Bookstore"}
                            {user.businessType === "ELECTRONICS" &&
                              "🔌 Electronics"}
