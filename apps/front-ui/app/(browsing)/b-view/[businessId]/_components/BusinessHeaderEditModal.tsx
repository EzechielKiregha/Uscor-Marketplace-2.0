		...
		try {
			...
			if (coverImageFile) {
				...
				const blob = await put(
					`business/${business.id}/cover/${Date.now()}-${coverImageFile.name}`,
					coverImageFile,
					{
						access: "public",
						token: blobToken,
			...
			if (avatarFile) {
				...
				const blob = await put(
					`business/${business.id}/avatar/${Date.now()}-${avatarFile.name}`,
					avatarFile,
					{
						access: "public",
						token: blobToken,
				...
				<div className="p-6">
					...
					<form onSubmit={handleSubmit} className="space-y-6">
						...
						<div>
							...
							<div className="relative h-48 rounded-lg overflow-hidden border hover:border-primary ">
								{coverImagePreview ? (
						...
						<div>
							...
							<div className="flex items-center gap-4">
								<div className="relative w-24 h-24 rounded-full overflow-hidden border hover:border-primary ">
									{avatarPreview ? (
										<div className="relative w-full h-full">
											<img
												src={avatarPreview}
