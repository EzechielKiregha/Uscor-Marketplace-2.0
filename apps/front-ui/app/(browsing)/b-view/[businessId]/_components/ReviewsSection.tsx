				...
				<div className="flex items-start gap-3">
					...
					<div>
						...
						{business.businessType === "HARDWARE" && (
							<p className="text-sm text-muted-foreground mt-1">
								Customers consistently praise {business.name} for their
								knowledgeable staff and quality hardware products. Many reviews
								highlight the helpful advice provided by their technicians when
								selecting tools and equipment for specific projects.
							</p>
						)}
						{business.businessType === "TRANSPORTATION" && (
							<p className="text-sm text-muted-foreground mt-1">
								{business.name} receives excellent feedback for their timely
								deliveries and professional drivers. Customers appreciate the
								real-time tracking and communication about delivery status
								throughout the process.
							</p>
						)}
						{business.businessType === "ARTISAN" && (
							<p className="text-sm text-muted-foreground mt-1">
								Customers love the unique, handcrafted products from{" "}
								{business.name}. Reviews frequently mention the attention to
								detail, quality craftsmanship, and the personal touch that comes
								with each custom order.
							</p>
						)}
						{business.businessType !== "HARDWARE" &&
							business.businessType !== "TRANSPORTATION" &&
							business.businessType !== "ARTISAN" && (
								<p className="text-sm text-muted-foreground mt-1">
									{business.name} maintains a strong reputation for excellent
									customer service and quality products/services. Their
									commitment to customer satisfaction is reflected in their
									positive reviews across the marketplace.
								</p>
							)}
					</div>
				</div>
