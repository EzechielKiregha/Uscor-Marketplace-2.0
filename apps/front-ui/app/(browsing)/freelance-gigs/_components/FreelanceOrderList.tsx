...
export default function FreelanceOrderList({
	clientId,
}: FreelanceOrderListProps) {
	...
	return (
		<div className="min-h-screen bg-white dark:bg-black text-gray-900 dark:text-gray-100 p-4 sm:p-6 md:p-8">
			...
			{data && data.freelanceOrders.length === 0 && (
			...
			{data?.freelanceOrders.map((order: any) => (
				...
				>
					...
					<CardContent>
						...
						<p className="text-sm text-gray-600 dark:text-gray-400">
							Status:{" "}
							<AnimatedBadge text={order.status} borderColor="via-amber-500" />
						</p>
