  ...
  dbPromise = openDB(DB_NAME, DB_VERSION, {
    upgrade(db) {
      ...
      if (!db.objectStoreNames.contains(STORES.OFFLINE_OPERATIONS)) {
        const offlineStore = db.createObjectStore(STORES.OFFLINE_OPERATIONS, { keyPath: 'id' });
        offlineStore.createIndex('timestamp', 'timestamp', { unique: false });
        offlineStore.createIndex('status', 'status', { unique: false });
        offlineStore.createIndex('operationName', 'operationName', { unique: false });
      }
      
      // Worker cache store
      if (!db.objectStoreNames.contains(STORES.WORKER_CACHE)) {
        const cacheStore = db.createObjectStore(STORES.WORKER_CACHE, { keyPath: 'key' });
        cacheStore.createIndex('timestamp', 'timestamp', { unique: false });
      }
      
      // Local sales store
      if (!db.objectStoreNames.contains(STORES.LOCAL_SALES)) {
        const salesStore = db.createObjectStore(STORES.LOCAL_SALES, { keyPath: 'id' });
      ...
      if (!db.objectStoreNames.contains(STORES.LOCAL_INVENTORY)) {
        const inventoryStore = db.createObjectStore(STORES.LOCAL_INVENTORY, { keyPath: 'id' });
      ...
      if (!db.objectStoreNames.contains(STORES.LOCAL_CHAT)) {
        const chatStore = db.createObjectStore(STORES.LOCAL_CHAT, { keyPath: 'id' });
...
export async function updateIndexedDB(storeName: string, key: string, data: any) {
  const db = await initDB();
...
export async function syncOfflineOperations() {
  const db = await initDB();
  const offlineOps = await db.getAll(STORES.OFFLINE_OPERATIONS, IDBKeyRange.only('PENDING'));
  ...
  for (const op of offlineOps) {
    try {
      ...
      await updateIndexedDB(STORES.OFFLINE_OPERATIONS, op.id, { status: 'SYNCED' });
    } catch (error) {
      console.error('Failed to sync operation:', op.id, error);
      await updateIndexedDB(STORES.OFFLINE_OPERATIONS, op.id, { status: 'FAILED' });
    }
  }
}
