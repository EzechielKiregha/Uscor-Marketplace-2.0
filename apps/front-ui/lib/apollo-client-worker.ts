...
const httpLink = process.env.NODE_ENV === 'production' 
  ? new HttpLink({ uri: 'https://uscor-marketplace-2-0-server.vercel.app/graphql' }) 
...
const errorLink = onError(({ graphQLErrors, networkError, operation, forward }) => {
  ...
  if (networkError) {
    console.warn('Network error occurred, attempting offline operation:', networkError);
    
    // For critical operations, save to IndexedDB for later sync
    if (operation.operationName.includes('Create') || 
...
const offlineLink = new ApolloLink((operation, forward) => {
  return new Observable(observer => {
    ...
    if (!isOnline) {
      ...
      if (operation.operationName.includes('Get') || operation.operationName.includes('Query')) {
      ...
      } else if (operation.operationName.includes('Create') || 
                 operation.operationName.includes('Update') || 
                 operation.operationName.includes('Add')) {
        ...
        saveToIndexedDB('offlineOperations', {
          ...
          const optimisticResponse = getOptimisticResponse(operationName, variables);
