...
export function useOfflinePOS(storeId: string, workerId: string, role: string) {
  ...
  const loadLocalSales = useCallback(async () => {
    try {
      ...
      const filtered = sales.filter((s: LocalSale) => s.storeId === storeId);
  ...
  const loadConflicts = useCallback(async () => {
    try {
      ...
      setConflicts(allConflicts.filter((c: SyncConflict) => !c.resolved));
    } catch {
      setConflicts([]);
    }
  }, []);
  ...
  const syncPendingOperations = useCallback(async () => {
    ...
    try {
      ...
      if (completedSales.length > 0) {
        const salesToSync = completedSales.map((op: OfflineOperation) => ({
          ...
          saleProducts: op.payload.saleProducts.map((sp: any) => ({
        ...
        try {
          ...
          for (const item of result.results) {
            const matchingOp = completedSales.find(
              (op: OfflineOperation) => op.payload.localId === item.localId,
            );

            if (!matchingOp) continue;

            if (item.status === "SYNCED" || item.status === "DUPLICATE") {
              await updateIndexedDB("offlineOperations", matchingOp.id, {
                status: "SYNCED",
              });
          ...
          await loadConflicts();
          return; // keep "error" status — don't fall through to "idle"
        }
      }
  ...
  useEffect(() => {
    ...
    // Don't just wait for the 'online' event — it won't fire if the app
    // was already online when this mounted, leaving pending ops stuck
    // until the 30s interval catches up.
    if (navigator.onLine) {
  ...
  const createSale = useCallback(
    async (saleWorkerId?: string, clientId?: string) => {
      ...
      showToast(
        "success",
        "Offline Sale",
        "Sale created locally — will sync when online",
      );
      return localId;
    },
    [
