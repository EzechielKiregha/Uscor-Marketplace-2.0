...
import type { FREELANCE_SERVICE_CATEGORIES } from "@/config/freelance-categories";
						...
						<div className="grid grid-cols-4 gap-x-8 gap-y-10 py-16">
							<div className="col-span-4 col-start-1 grid-cols-3 grid gap-x-8">
								{category.services.map((item) => (
									<div
										key={item.name}
										className=" text-base space-y-2 sm:text-sm"
									>
										...
										<Link
											// onClick={() => nav()}
											href={item.href}
											className="mt-6 font-medium text-gray-200"
										>
											{item.name}
