		...
		<>
			...
			{isOpen && (
				<div className="fixed inset-0 z-50 lg:hidden">
					...
					<div
						ref={drawerRef}
						className="absolute inset-y-0 left-0 w-4/5 max-w-sm bg-card shadow-xl border-r border-border"
					>
						...
						<div className="flex-1 overflow-y-auto p-4">
							{FREELANCE_SERVICE_CATEGORIES.map((category) => (
								<div key={category.label} className="mb-6 last:mb-0">
									<h3 className="px-3 text-sm font-semibold text-foreground mb-3">
										{category.label}
									</h3>
			...
			{FREELANCE_SERVICE_CATEGORIES.map((category, i) => {
				...
				return (
					<div key={i} className="relative">
						...
						{isOpen && (
							<div className="absolute left-0 top-full mt-2 w-80 rounded-xl bg-card border border-border hover:border-primary hover:bg-primary/5 shadow-lg overflow-hidden z-10">
