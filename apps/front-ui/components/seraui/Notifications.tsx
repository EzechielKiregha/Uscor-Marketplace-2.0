...
function NotificationsPopover() {
  ...
  const buildLink = (r: string | undefined, chatId: string) => {
    if (r === "business")
      return `/business/dashboard/chats?notif_chat_id=${chatId}`;
  ...
  return (
    <Popover>
      <PopoverTrigger>
        {/* <div className="relative cursor-pointer text-sm font-medium text-gray-600 dark:text-gray-300 hover:text-orange-500 dark:hover:text-orange-400 transition-colors">
          <Bell className="h-5 w-5" />
        </div> */}
        <Button
          variant="ghost"
          size="sm"
          className="relative cursor-pointer text-sm font-medium text-gray-600 dark:text-gray-300 hover:text-orange-500 dark:hover:text-orange-400 transition-colors"
        >
          <Bell className="h-5 w-5" />
          {totalUnread > 0 && (
            <span className="absolute -top-1 -right-1 text-xs bg-destructive text-white px-1 rounded-full">
              {totalUnread}
            </span>
          )}
        </Button>
          ...
          {!loading &&
            items.map((n: any) => (
              <div
                ...
                className="block px-3 py-2 hover:bg-muted/50 cursor-pointer"
              >
                <div className="flex items-center justify-between">
                  <div className="text-sm truncate">
                    New message in chat {n.chatId}
                  </div>
