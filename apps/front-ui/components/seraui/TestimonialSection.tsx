...
const TestimonialSection: React.FC<TestimonialSectionProps> = ({
  ...
  return (
    <div className={`font-inter flex flex-col items-center py-16 px-4 sm:px-6 lg:px-8 text-(--secondary-dark) dark:text-(--lightGray) ${className}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 w-full max-w-6xl">
        <div className="bg-card/80 /50 p-8 rounded-xl flex flex-col justify-between border border-(--secondary-light) dark:border-(--secondary-dark) backdrop-blur-sm shadow-md">
          <div className="mb-8">
            {testimonials[0].logo && (
            ...
            <p className="text-base sm:text-lg lg:text-xl text-(--secondary-dark) dark:text-(--lightGray) leading-relaxed mb-8">
          ...
          <div className="flex items-center">
            ...
            />
            <div>
              <p className="font-semibold text-(--secondary-dark) dark:text-(--lightGray)">{testimonials[0].author}</p>
              <p className="text-sm text-(--secondary-light) dark:text-(--secondary-dark)">{testimonials[0].role}</p>
            </div>
          </div>
        </div>
        <div className="flex flex-col gap-4">
          <div className="bg-card/80 /50 p-8 rounded-xl flex flex-col justify-between border border-(--secondary-light) dark:border-(--secondary-dark) backdrop-blur-sm shadow-md">
            ...
            <div className="flex items-center">
              ...
              />
              <div>
                <p className="font-semibold text-(--secondary-dark) dark:text-(--lightGray)">{testimonials[1].author}</p>
                <p className="text-sm text-(--secondary-light) dark:text-(--secondary-dark)">{testimonials[1].role}</p>
              </div>
            </div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {testimonials.slice(2).map((testimonial) => (
              <div key={testimonial.author} className="bg-card/80 /50 p-6 rounded-xl flex flex-col justify-between border border-(--secondary-light) dark:border-(--secondary-dark) backdrop-blur-sm shadow-md">
                <p className="text-sm sm:text-base text-(--secondary-dark) dark:text-(--lightGray) leading-relaxed mb-6">
                  {testimonial.quote}
                </p>
                <div className="flex items-center">
                  <img
                  ...
                  />
                  <div>
                    <p className="font-semibold text-(--secondary-dark) dark:text-(--lightGray)">{testimonial.author}</p>
                    <p className="text-sm text-(--secondary-light) dark:text-(--secondary-dark)">{testimonial.role}</p>
                  </div>
                </div>
              </div>
            ))}
